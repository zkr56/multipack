...
# ══════════════════════════════════════════════════════════════
# SYSTÈME DE CONNEXION — MULTIPACK SA
# Rôles : DG · Directeur Commercial · Contrôleur de Gestion · Production
# ══════════════════════════════════════════════════════════════
...
UTILISATEURS = {
    "dg@multipack.ci": {
        ...
        "role": "DG",
        "mdp": "multipack2024",
        "pages_autorisees": "ALL",
        "couleur": "#4F6EF7",
        "icone": "👔",
        "description": "Accès complet à tous les modules",
    },
    "commercial@multipack.ci": {
        "nom": "Directeur Commercial",
        "role": "Commercial",
        "mdp": "ventes2024",
        "pages_autorisees": [
            "🏠  Accueil","📊  Vue d'ensemble","💰  Chiffre d'Affaires",
            "🛍️  Nos Produits","👥  Nos Clients","🎯  Segmentation RFM",
            "💳  Paiements","🔮  Prévisions","💡  Rentabilisation","⚠️  Alertes",
        ],
        "couleur": "#10B981",
        "icone": "💼",
        "description": "CA, Clients, RFM, Paiements, Prévisions",
    },
    "cg@multipack.ci": {
        ...
        "pages_autorisees": [
            "🏠  Accueil","📊  Vue d'ensemble","💰  Chiffre d'Affaires",
            "🧮  Contrôle de Gestion","🔮  Prévisions","💳  Paiements",
            "🛒  Achats Matières","📦  Stocks & Inventaire","⚠️  Alertes",
        ],
        "couleur": "#F59E0B",
        "icone": "📊",
        "description": "P&L, Budget, BFR, DSO, Point mort, Achats",
    },
    "production@multipack.ci": {
        "nom": "Responsable Production",
        "role": "Production",
        "mdp": "usine2024",
        "pages_autorisees": [
            "🏠  Accueil","🏭  Production","📦  Stocks & Inventaire",
            "🛒  Achats Matières","🔮  Prévisions","⚠️  Alertes",
        ],
        "couleur": "#7C3AED",
        "icone": "⚙️",
        "description": "Production, Stocks, Achats, Prévisions",
    },
}

def _login_page():
    """Page de connexion — affichée quand non connecté."""
    # CSS dédié page login — appliqué UNIQUEMENT ici, sans position:fixed
    st.markdown("""
    <style>
    [data-testid="stSidebar"] { display: none !important; }
    .main .block-container { padding: 2rem 1rem !important; max-width: 100% !important; }
    div[data-testid="stVerticalBlock"] > div:first-child { background: transparent !important; }
    </style>
    """, unsafe_allow_html=True)

    # Fond foncé via CSS body (pas de position:fixed)
    st.markdown("""
    <style>
    .stApp { background: linear-gradient(135deg, #0F172A 0%, #1E293B 60%, #0F172A 100%) !important; }
    </style>
    """, unsafe_allow_html=True)
    ...
    with col_center:
        ...
        <div style="text-align:center; margin-bottom: 28px;">
            ...
            <div style="font-size:1.8rem; font-weight:900; color:white;
                        letter-spacing:-0.04em; margin-bottom:6px;">MULTIPACK SA</div>
            <div style="font-size:0.78rem; color:#64748B; font-weight:600;
                        text-transform:uppercase; letter-spacing:0.1em; margin-bottom:4px;">
                Tableau de Bord · Direction Générale
            </div>
            <div style="font-size:0.72rem; color:#334155; margin-bottom:20px;">
        ...
        with st.form("login_form", clear_on_submit=False):
            email = st.text_input(
                "📧 Email",
                placeholder="votre.nom@multipack.ci",
            )
        ...
        if submitted:
            if email in UTILISATEURS and UTILISATEURS[email]["mdp"] == password:
                user = UTILISATEURS[email]
                st.session_state["logged_in"]  = True
                st.session_state["user_email"] = email
                st.session_state["user_nom"]   = user["nom"]
                st.session_state["user_role"]  = user["role"]
                st.session_state["user_pages"] = user["pages_autorisees"]
                st.session_state["user_color"] = user["couleur"]
                st.session_state["user_icon"]  = user["icone"]
                st.rerun()
        ...
        <div style="margin-top:20px; padding:16px;
                    ...
                    border-radius:14px; font-size:0.73rem;
                    color:#64748B; line-height:2;">
            <div style="font-weight:800; color:#475569; margin-bottom:8px;
                        font-size:0.68rem; text-transform:uppercase; letter-spacing:0.1em;">
                🔐 Comptes de démonstration
            </div>
            <div>👔 <span style="color:#4F6EF7; font-weight:700;">DG</span>
                 &nbsp;·&nbsp; dg@multipack.ci &nbsp;/&nbsp; multipack2024</div>
            <div>💼 <span style="color:#10B981; font-weight:700;">Commercial</span>
                 &nbsp;·&nbsp; commercial@multipack.ci &nbsp;/&nbsp; ventes2024</div>
            <div>📊 <span style="color:#F59E0B; font-weight:700;">Contrôle de Gestion</span>
                 &nbsp;·&nbsp; cg@multipack.ci &nbsp;/&nbsp; gestion2024</div>
            <div>⚙️ <span style="color:#7C3AED; font-weight:700;">Production</span>
                 &nbsp;·&nbsp; production@multipack.ci &nbsp;/&nbsp; usine2024</div>
        </div>
        """, unsafe_allow_html=True)
...
# ── Utilisateur connecté — récupération du profil ─────────────
_USER_PAGES  = st.session_state.get("user_pages", "ALL")
_USER_NOM    = st.session_state.get("user_nom", "Utilisateur")
_USER_ROLE   = st.session_state.get("user_role", "DG")
_USER_COLOR  = st.session_state.get("user_color", "#4F6EF7")
_USER_ICON   = st.session_state.get("user_icon", "👤")

def page_autorisee(page_name):
    if _USER_PAGES == "ALL": return True
    return page_name in _USER_PAGES
...
.rfm-badge {{display:inline-block;padding:4px 12px;border-radius:20px;
    font-size:0.72rem;font-weight:800;text-transform:uppercase;letter-spacing:0.07em;}}
.score-card {{background:linear-gradient(135deg,#0F172A,#1E293B);border-radius:20px;
    padding:28px 32px;border:1px solid rgba(79,110,247,0.2);
    box-shadow:0 8px 32px rgba(0,0,0,0.2);text-align:center;color:white;}}
.score-card .sc-num {{font-size:4rem;font-weight:900;line-height:1;
    letter-spacing:-0.05em;margin:12px 0 6px;}}
.score-card .sc-lbl {{font-size:0.7rem;color:#64748B;text-transform:uppercase;letter-spacing:0.1em;}}
.score-card .sc-sub {{font-size:0.82rem;color:#94A3B8;margin-top:8px;}}
.anomaly-row {{background:#FFFFFF;border:1px solid {C['bordure']};border-radius:12px;
    padding:14px 18px;margin:6px 0;display:flex;align-items:center;
    justify-content:space-between;box-shadow:0 1px 4px rgba(0,0,0,0.04);}}
.renta-card {{background:{C['blanc']};border-radius:16px;border:1px solid {C['bordure']};
    padding:22px 24px;box-shadow:0 2px 8px rgba(0,0,0,0.06);height:100%;position:relative;overflow:hidden;}}
.renta-card::before {{content:'';position:absolute;top:0;left:0;width:100%;height:3px;border-radius:16px 16px 0 0;}}
.rc-vert::before   {{background:linear-gradient(90deg,{C['vert']},{C['cyan']});}}
.rc-bleu::before   {{background:linear-gradient(90deg,{C['bleu']},{C['violet']});}}
.rc-orange::before {{background:linear-gradient(90deg,{C['orange']},{C['rouge']});}}
.rc-violet::before {{background:linear-gradient(90deg,{C['violet']},{C['rose']});}}
.rc-or::before     {{background:linear-gradient(90deg,{C['or']},{C['orange']});}}
.rc-step {{display:flex;gap:8px;align-items:flex-start;
    font-size:0.76rem;color:{C['muted']};margin-bottom:6px;}}
.rs-num {{min-width:18px;height:18px;border-radius:50%;background:{C['bleu']};color:white;
...
def _build_features(idx_arr, n_hist):
    """
    Construit la matrice de features pour la prévision :
    - Tendance (t, t²)
    - Saisonnalité mensuelle (sin/cos harmoniques 1 et 2)
    - Lags (si suffisamment de données)
    """
    t     = idx_arr.astype(float)
    sin1  = np.sin(2 * np.pi * t / 12)
    cos1  = np.cos(2 * np.pi * t / 12)
    sin2  = np.sin(4 * np.pi * t / 12)
    cos2  = np.cos(4 * np.pi * t / 12)
    return np.column_stack([t, t**2, sin1, cos1, sin2, cos2])


def prevoir_ca(serie: pd.Series, n: int = 6):
    """
    Modèle de prévision amélioré :
    - Ridge Regression avec features saisonnières + tendance quadratique
    - Saisonnalité capturée via harmoniques Fourier (sin/cos)
    - Intervalle de confiance via bootstrap résiduel (percentile 5/95)
    - Validation croisée temporelle pour sélectionner l'alpha Ridge optimal
    - Minimum 6 points d'historique requis
    """
    y = serie.values.astype(float)
    T = len(y)

    if T < 6:
        # Trop peu de données → fallback moyenne mobile
        last  = serie.index[-1]
        peri  = [str(pd.Period(last, "M") + i + 1) for i in range(n)]
        moy   = float(np.mean(y))
        std_f = float(np.std(y)) if T > 1 else moy * 0.1
        y_fut = np.full(n, moy)
        return pd.DataFrame({
            "Période":  peri,
            "CA_Prévu": y_fut,
            "Bas":      np.maximum(y_fut - 1.5 * std_f, 0),
            "Haut":     y_fut + 1.5 * std_f,
        })
    ...
    # ── Features historique ────────────────────────────────
    idx_hist = np.arange(T)
    X_hist   = _build_features(idx_hist, T)

    # ── Sélection alpha Ridge par validation croisée temporelle ──
    best_alpha, best_score = 1.0, np.inf
    for alpha in [0.01, 0.1, 1.0, 10.0, 100.0, 500.0]:
        mdl_cv = Ridge(alpha=alpha)
        if T >= 12:
            tscv   = TimeSeriesSplit(n_splits=min(4, T // 4))
            scores = -cross_val_score(mdl_cv, X_hist, y_norm,
                                       cv=tscv, scoring="neg_mean_squared_error")
            score  = scores.mean()
        else:
            mdl_cv.fit(X_hist, y_norm)
            score = np.mean((y_norm - mdl_cv.predict(X_hist)) ** 2)
        if score < best_score:
            best_score = score
            best_alpha = alpha
    ...
    model.fit(X_hist, y_norm)
    y_hat_norm = model.predict(X_hist)
    residuals  = y_norm - y_hat_norm

    # ── Prévision future ────────────────────────────────────
    idx_fut = np.arange(T, T + n)
    X_fut   = _build_features(idx_fut, T)
    y_fut_norm = model.predict(X_fut)
    y_fut  = y_fut_norm * y_std + y_mean
    y_fut  = np.maximum(y_fut, 0)

    # ── Intervalle bootstrap (percentile 5–95) ──────────────
    np.random.seed(42)
    n_boot = 500
    boot_preds = np.zeros((n_boot, n))
    for b in range(n_boot):
        noise       = np.random.choice(residuals, size=n, replace=True)
        boot_preds[b] = y_fut_norm + noise
    boot_preds   = boot_preds * y_std + y_mean
    boot_preds   = np.maximum(boot_preds, 0)
    y_bas  = np.percentile(boot_preds, 5,  axis=0)
    y_haut = np.percentile(boot_preds, 95, axis=0)
...
def generer_rapport_pptx(df_fact, df_inv, df_ent, df_sor, df_prod, df_f, now):
...
with st.sidebar:
    ...
    <div style="text-align:center;padding:18px 0 10px;">
        ...
        <div style="font-size:0.65rem;color:#475569;margin-top:2px;">Zone Ind. Yopougon · Abidjan, CI</div>
    </div>
    <div style="background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.08);
                border-radius:12px;padding:12px 14px;margin:0 0 14px;">
        <div style="display:flex;align-items:center;gap:10px;">
            <div style="width:36px;height:36px;border-radius:50%;
                        background:{_USER_COLOR};display:flex;align-items:center;
                        justify-content:center;font-size:1.1rem;flex-shrink:0;">{_USER_ICON}</div>
            <div>
                <div style="font-size:0.82rem;font-weight:700;color:white;">{_USER_NOM}</div>
                <div style="font-size:0.65rem;color:#64748B;margin-top:1px;">
                    Rôle : <span style="color:{_USER_COLOR};font-weight:700;">{_USER_ROLE}</span>
                </div>
            </div>
        </div>
    </div>
    <div style="font-size:0.62rem;font-weight:700;color:#334155;text-transform:uppercase;
                letter-spacing:0.1em;padding:0 4px 8px;">🔧 Navigation & Filtres</div>
    """, unsafe_allow_html=True)
    ...
    PAGES_VISIBLES = ALL_PAGES if _USER_PAGES == "ALL" else [p for p in ALL_PAGES if p in _USER_PAGES]
    page = st.radio("Page", PAGES_VISIBLES, label_visibility="collapsed")
    ...
    <div style="margin-top:12px;font-size:0.63rem;color:#334155;text-align:center;line-height:1.7;">
    ...
    <div style="border-top:1px solid rgba(255,255,255,0.06);margin:14px 0 12px;"></div>
    <div style="font-size:0.62rem;font-weight:700;color:#334155;text-transform:uppercase;
                letter-spacing:0.1em;padding:0 4px 6px;">🔑 Clé API Anthropic</div>
    """, unsafe_allow_html=True)
    api_key_input = st.text_input(
        ...
        label_visibility="collapsed",
        help="Votre clé API Anthropic pour activer le chatbot IA. Trouvez-la sur console.anthropic.com",
        value=st.session_state.get("anthropic_api_key", ""),
    )
    if api_key_input:
        st.session_state["anthropic_api_key"] = api_key_input
        st.markdown("""
        <div style="font-size:0.67rem;color:#10B981;padding:4px 2px;">✅ Clé enregistrée</div>
        """, unsafe_allow_html=True)
    else:
        st.markdown("""
        <div style="font-size:0.67rem;color:#64748B;padding:4px 2px;line-height:1.5;">
        Requis pour le <b>💬 Conseiller IA</b><br>
...
NAV_ITEMS = [
    ...
    ("🛒  Achats Matières",    "🛒","Achats"),
    ("🤖  IA & Anomalies",     "🤖","IA"),
    ("📈  Comparaisons",       "📈","Comparaisons"),
    ("🧮  Contrôle de Gestion","🧮","Contrôle"),
    ("👷  Pilotage RH",        "👷","RH"),
    ("💡  Rentabilisation",    "💡","Rentabilité"),
    ("⚠️  Alertes",            "⚠️","Alertes"),
]
...
# ══════════════════════════════════════════════════════════════
    ...
    with tab1:
        ...
        # Calcul R² via Ridge (même modèle que prevoir_ca)
        X_arr_feat = _build_features(np.arange(len(y_arr)), len(y_arr))
        _mdl_r2 = Ridge(alpha=1.0).fit(X_arr_feat, y_arr)
        y_pred_in = _mdl_r2.predict(X_arr_feat)
...
# ══════════════════════════════════════════════════════════════
# CSS SUPPLÉMENTAIRE — Nouvelles pages
# ══════════════════════════════════════════════════════════════
...
def calculer_rfm(df):
    ...
    rfm = df.groupby("CLIENT").agg(
        ...
        Fréquence=("MONTANT TTC","count"),
        Montant=("MONTANT TTC","sum"),
        Segment=("SEGMENT","first"),
        Zone=("ZONE","first"),
    ).reset_index()
    for col, labels in [("Récence",[5,4,3,2,1]),("Fréquence",[1,2,3,4,5]),("Montant",[1,2,3,4,5])]:
        try:
            rfm[col[0]+"_Score"] = pd.qcut(rfm[col],5,labels=labels,duplicates="drop").astype(float)
        except Exception:
            rfm[col[0]+"_Score"] = 3.0
    rfm = rfm.fillna(3)
    def seg(row):
        r = row.get("R_Score",3); f = row.get("F_Score",3); m = row.get("M_Score",3)
        if r>=4 and f>=4 and m>=4: return "Champions"
        elif r>=3 and f>=3:        return "Fidèles"
        elif r>=3 and m>=3:        return "Potentiel"
        elif r<=2 and f>=2:        return "A Risque"
        else:                      return "Dormants"
    rfm["Catégorie"] = rfm.apply(seg, axis=1)
    ACTS = {
        "Champions":"Fidéliser, programme VIP, remise volume.",
        "Fidèles":"Upsell — proposer nouvelles références ou volumes.",
        "Potentiel":"Augmenter fréquence de contact, offre promotionnelle.",
        "A Risque":"Appel urgent, comprendre la cause, offrir ristourne.",
        "Dormants":"Email réactivation, offre de retour spéciale.",
    }
    rfm["Action"] = rfm["Catégorie"].map(ACTS)
    rfm["CA_Pot"]  = rfm.apply(lambda r:
        r["Montant"]*1.30 if r["Catégorie"]=="Champions" else
        r["Montant"]*1.15 if r["Catégorie"]=="Fidèles"   else
        r["Montant"]*0.50 if r["Catégorie"]=="Dormants"  else r["Montant"], axis=1)
    rfm["RFM_Moy"] = (rfm.get("R_Score",3)+rfm.get("F_Score",3)+rfm.get("M_Score",3))/3
    return rfm.sort_values("RFM_Moy",ascending=False).reset_index(drop=True)
...
def detecter_anomalies(df):
    ...
    df2["JOUR"]    = df2["DATE"].dt.dayofweek
    feat = df2[["MONTANT TTC","MOIS","JOUR"]].dropna()
    X = StandardScaler().fit_transform(feat)
    clf = IsolationForest(n_estimators=200,contamination=0.05,random_state=42)
    df2.loc[feat.index,"Flag"]  = clf.fit_predict(X)
    df2.loc[feat.index,"Score"] = -clf.score_samples(X)
    anom = df2[df2["Flag"]==-1].copy()
    q95 = df["MONTANT TTC"].quantile(0.95); q05 = df["MONTANT TTC"].quantile(0.05)
    anom["Raison"] = anom.apply(lambda r:
        "Montant exceptionnel (haut)" if r["MONTANT TTC"]>q95 else
        "Montant anormal (bas)"       if r["MONTANT TTC"]<q05 else
        "Comportement inhabituel", axis=1)
    q80 = anom["Score"].quantile(0.8) if len(anom)>1 else 0
    q50 = anom["Score"].quantile(0.5) if len(anom)>1 else 0
    anom["Niveau"] = anom["Score"].apply(
        lambda v: "Critique" if v>q80 else ("Modere" if v>q50 else "Faible"))
    return anom.sort_values("Score",ascending=False)


@st.cache_data(show_spinner=False)
def predire_impayes(df):
    ...
    df2["target"]  = (df2["ETAT DE PAIEMENT"]=="Impayée").astype(int)
    df2["log_m"]   = np.log1p(df2["MONTANT TTC"])
    le_s = LabelEncoder().fit(df2["SEGMENT"])
    le_z = LabelEncoder().fit(df2["ZONE"])
    df2["seg_e"] = le_s.transform(df2["SEGMENT"])
    df2["zon_e"] = le_z.transform(df2["ZONE"])
    cols = ["log_m","MOIS","ANNEE","seg_e","zon_e"]
    X = df2[cols].dropna(); y = df2.loc[X.index,"target"]
    clf = RandomForestClassifier(n_estimators=300,max_depth=6,
                                  class_weight="balanced",random_state=42)
    clf.fit(X,y)
    auc = cross_val_score(clf,X,y,cv=5,scoring="roc_auc").mean()
    df2.loc[X.index,"Proba_Imp"] = clf.predict_proba(X)[:,1]
    imp = pd.Series(clf.feature_importances_,index=cols)
    return df2, auc, imp


def score_sante(df_f, df_inv, df_prod):
    ca  = df_f["MONTANT TTC"].sum()
    pay = df_f[df_f["ETAT DE PAIEMENT"]=="Payée"]["MONTANT TTC"].sum()
    tr  = pay/ca*100 if ca>0 else 0
    rd  = df_prod["Taux rendement"].mean()
    nr  = (df_inv["Statut"]=="Non disponible").sum()
    nt  = max(len(df_inv),1)
    nf  = (df_inv["Statut"]=="Stock faible").sum()
    dm  = df_f.copy(); dm["P"] = dm["DATE"].dt.to_period("M").astype(str)
    cm  = dm.groupby("P")["MONTANT TTC"].sum()
    te  = (cm.iloc[-1]/cm.iloc[-3]*100 if len(cm)>=3 and cm.iloc[-3]>0 else 100)
    sc  = {"Recouvrement":min(100,tr*100/80),"Production":min(100,rd*100/85),
           "Stocks":max(0,100-(nr/nt*100*5)-(nf/nt*100*2)),"Croissance":min(100,max(0,te))}
    tot = sc["Recouvrement"]*0.35+sc["Production"]*0.20+sc["Stocks"]*0.20+sc["Croissance"]*0.25
    return round(tot,1), sc
...
if page == "🎯  Segmentation RFM":
    ...
    st.markdown(f"""
    <div style="background:linear-gradient(135deg,#0F172A,#1E293B,{C['vert']});
                border-radius:14px;padding:22px 28px;margin-bottom:18px;
                border:1px solid rgba(16,185,129,0.3);">
      <div style="font-size:1.15rem;font-weight:800;color:white;margin-bottom:4px;">
        🎯 Segmentation RFM — Connaissance Client Avancée</div>
      <div style="font-size:0.8rem;color:rgba(255,255,255,0.6);">
        Récence · Fréquence · Montant · Action recommandée par client</div>
    </div>""", unsafe_allow_html=True)

    comment("La segmentation <b>RFM</b> classe chaque client selon 3 critères : "
            "<b>Récence</b> (quand a-t-il commandé ?), <b>Fréquence</b> (combien de fois ?), "
            "<b>Montant</b> (combien dépensé ?). 5 catégories actionnables, une action précise pour chacune.","🎯 Qu'est-ce que le RFM ?")

    with st.spinner("Calcul RFM…"):
        rfm = calculer_rfm(df_f)

    cats = rfm["Catégorie"].value_counts()
    CMAP = {"Champions":C["vert"],"Fidèles":C["bleu"],"Potentiel":C["orange"],
            "A Risque":C["rouge"],"Dormants":C["violet"]}
    CICO = {"Champions":"🏆","Fidèles":"💛","Potentiel":"🌱","A Risque":"⚠️","Dormants":"😴"}
    ORDER = ["Champions","Fidèles","Potentiel","A Risque","Dormants"]
    cols5 = st.columns(5)
    for cst, cat in zip(cols5, ORDER):
        nb  = int(cats.get(cat,0))
        ca_c= rfm[rfm["Catégorie"]==cat]["Montant"].sum()
        kc  = {"Champions":"vert","Fidèles":"bleu","Potentiel":"orange","A Risque":"rouge","Dormants":"violet"}
        with cst: kpi(CICO[cat], cat, str(nb), fmt(ca_c,"").strip(), kc[cat])
    ...
    with c_a:
        ...
        fig_p = go.Figure(go.Pie(
            labels=[CICO[c]+" "+c for c in cats.index],
            values=cats.values, hole=0.55,
            marker=dict(colors=[CMAP.get(c,C["bleu"]) for c in cats.index],
                        line=dict(color="white",width=2)),
            textinfo="label+percent",textfont=dict(size=10),
            hovertemplate="<b>%{label}</b><br>%{value} clients (%{percent})<extra></extra>"))
        fig_p.update_layout(paper_bgcolor="#FFFFFF",height=300,
            margin=dict(t=10,b=30,l=10,r=10),
            legend=dict(orientation="h",y=-0.08,xanchor="center",x=0.5,font=dict(size=9)))
        st.plotly_chart(fig_p, use_container_width=True)
    with c_b:
        section("💰  CA PAR CATÉGORIE")
        cca = rfm.groupby("Catégorie")["Montant"].sum().reindex(ORDER).dropna().reset_index()
        fig_cb = go.Figure(go.Bar(
            x=cca["Montant"],y=cca["Catégorie"],orientation="h",
            marker_color=[CMAP.get(c,C["bleu"]) for c in cca["Catégorie"]],
            text=cca["Montant"].apply(lambda v: fmt(v,"").strip()),
            textposition="outside",textfont=dict(size=10),
            hovertemplate="<b>%{y}</b><br>%{x:,.0f} FCFA<extra></extra>"))
        fig_cb = excel_style(fig_cb,300,False)
        fig_cb.update_xaxes(tickformat=",.0f")
    ...
    section("🗺️  CARTE RFM — Positionnement de chaque client")
    comment("Chaque bulle = un client. Plus à droite = achète souvent. Plus haut = dépense plus. Champions = haut à droite.")
    fig_sc = go.Figure()
    for cat, grp in rfm.groupby("Catégorie"):
        fig_sc.add_trace(go.Scatter(
            x=grp["Fréquence"],y=grp["Montant"],mode="markers",name=cat,
            marker=dict(size=14,color=CMAP.get(cat,C["bleu"]),opacity=0.80,
                        line=dict(color="white",width=1.5)),
            text=grp["CLIENT"],
            hovertemplate="<b>%{text}</b><br>%{x} cmd · %{y:,.0f} FCFA<extra></extra>"))
    fig_sc = excel_style(fig_sc,380)
    fig_sc.update_xaxes(title="Fréquence (nb commandes)")
    fig_sc.update_yaxes(title="Montant total (FCFA)",tickformat=",.0f")
    st.plotly_chart(fig_sc, use_container_width=True)

    section("📋  PLAN D'ACTION PAR CLIENT")
    filtre = st.multiselect("Filtrer",ORDER,default=ORDER)
    show = rfm[rfm["Catégorie"].isin(filtre)].copy()
    disp = show[["CLIENT","Catégorie","Récence","Fréquence","Montant","Action","CA_Pot"]].copy()
    disp["Montant"] = disp["Montant"].apply(lambda v: f"{v:,.0f} FCFA")
    disp["CA_Pot"]  = disp["CA_Pot"].apply(lambda v: f"{v:,.0f} FCFA")
    disp["Récence"] = disp["Récence"].apply(lambda v: f"{v} j")
    st.dataframe(disp.rename(columns={"CLIENT":"Client","CA_Pot":"CA Potentiel",
        "Action":"Action Recommandée"}), use_container_width=True, height=380)

    ga = rfm[rfm["Catégorie"].isin(["Champions","Fidèles"])]["Montant"].sum()*0.15
    gr = rfm[rfm["Catégorie"].isin(["A Risque","Dormants"])]["Montant"].sum()*0.40
    prevision_comment(f"Potentiel : <b>+{fmt(ga)}</b> (upsell Champions/Fidèles) + "
        f"<b>+{fmt(gr)}</b> (réactivation). Total : <b>+{fmt(ga+gr)}</b>.","💰 Potentiel commercial")



# ══════════════════════════════════════════════════════════════
# PAGE ACHATS MATIÈRES PREMIÈRES — Planification complète
# ══════════════════════════════════════════════════════════════
elif page == "🛒  Achats Matières":

    st.markdown(f"""
    <div style="background:linear-gradient(135deg,#0F172A,#1E293B,{C['orange']});
                border-radius:14px;padding:22px 28px;margin-bottom:18px;
                border:1px solid rgba(245,158,11,0.35);">
      <div style="font-size:1.15rem;font-weight:800;color:white;margin-bottom:4px;">
        🛒 Planification des Achats Matières Premières</div>
      <div style="font-size:0.8rem;color:rgba(255,255,255,0.65);">
        Besoins estimés · Plan de commande · Budget matières · Alertes ruptures</div>
    </div>""", unsafe_allow_html=True)

    comment(
        "Cette page calcule automatiquement <b>quand commander</b> et <b>combien</b> de chaque matière première, "
        "en combinant les prévisions de CA, le taux de consommation historique et les stocks actuels. "
        "L'objectif : ne jamais être en rupture sans sur-stocker inutilement.",
        "🛒 Comment utiliser cette page ?"
    )

    # ── Paramètres de planification ──────────────────────────────
    with st.expander("⚙️ Paramètres de planification — Cliquez pour ajuster"):
        pp1, pp2, pp3 = st.columns(3)
        with pp1:
            horizon_achat = st.slider("Horizon de planification (mois)", 1, 6, 3, 1)
            stock_securite_pct = st.slider("Stock de sécurité (%)", 10, 50, 20, 5,
                help="% de consommation prévue à garder en réserve")
        with pp2:
            delai_livraison = st.slider("Délai livraison fournisseur (jours)", 5, 45, 15, 5)
            taux_matieres_ca = st.slider("Coût matières / CA HT (%)", 30, 60, 42, 2)
        with pp3:
            st.markdown("**Référence budget mensuel (FCFA)**")
            budget_matieres = st.number_input("Budget mensuel matières",
                value=int(df_f["MONTANT TTC"].sum()/1.18/36*0.42),
                step=100_000, format="%d")

    # ── Calcul des besoins ────────────────────────────────────────
    # 1. Prévision CA pour l'horizon
    df_all_a = df_f.copy()
    df_all_a["Période"] = df_all_a["DATE"].dt.to_period("M").astype(str)
    serie_ca = df_all_a.groupby("Période")["MONTANT TTC"].sum().sort_index()
    prev_ca_df = prevoir_ca(serie_ca, horizon_achat)
    prev_ca_total = prev_ca_df["CA_Prévu"].sum()
    prev_ca_ht   = prev_ca_total / 1.18
    budget_besoin = prev_ca_ht * (taux_matieres_ca / 100)

    # 2. Répartition par catégorie de matière (basée sur les achats historiques)
    mat_cats = df_ent[df_ent["Catégorie"]=="Matières Premières"].copy() if "Catégorie" in df_ent.columns else pd.DataFrame()
    
    # Parts par matière (historique ou hypothèse)
    MATIERES = [
        {"ref":"MAT-PE-001",  "nom":"Granulés PE-HD",      "cat":"Polyéthylène",    "part":0.35,
         "prix_unit":14000, "unite":"kg", "delai":delai_livraison, "fournisseur":"CHIMIE CI"},
        {"ref":"MAT-PP-001",  "nom":"Granulés PP",          "cat":"Polypropylène",   "part":0.28,
         "prix_unit":15500, "unite":"kg", "delai":delai_livraison, "fournisseur":"PLASTICO SA"},
        {"ref":"MAT-PVC-001", "nom":"Granulés PVC Souple",  "cat":"PVC",             "part":0.18,
         "prix_unit":18000, "unite":"kg", "delai":delai_livraison+5, "fournisseur":"CHIMIE CI"},
        {"ref":"MAT-REC-001", "nom":"Plastique Recyclé Mix","cat":"Recyclé",         "part":0.10,
         "prix_unit":7500,  "unite":"kg", "delai":delai_livraison-5, "fournisseur":"RECYPLAST CI"},
        {"ref":"MAT-COL-001", "nom":"Masterbatch Noir",     "cat":"Colorants",       "part":0.05,
         "prix_unit":22000, "unite":"kg", "delai":delai_livraison+10, "fournisseur":"PIGMENTS AFRIQUE"},
        {"ref":"MAT-COL-002", "nom":"Masterbatch Vert",     "cat":"Colorants",       "part":0.04,
         "prix_unit":23000, "unite":"kg", "delai":delai_livraison+10, "fournisseur":"PIGMENTS AFRIQUE"},
    ]

    # 3. Pour chaque matière : calculer besoin, stock dispo, quantité à commander
    plan_rows = []
    for m in MATIERES:
        besoin_total_fcfa = budget_besoin * m["part"]
        besoin_kg         = besoin_total_fcfa / m["prix_unit"]
        securite_kg       = besoin_kg * stock_securite_pct / 100
        
        # Stock actuel de cette matière
        inv_row = df_inv[df_inv["ref"] == m["ref"]]
        stock_actuel = int(inv_row["Stock final"].iloc[0]) if len(inv_row)>0 else 0
        valeur_stock = stock_actuel * m["prix_unit"]
        
        # Stock couverture (en mois)
        conso_mois = max(1, besoin_kg / horizon_achat)
        couverture_mois = stock_actuel / conso_mois if conso_mois > 0 else 0

        # Quantité à commander = besoin + sécurité - stock actuel
        qte_commander = max(0, besoin_kg + securite_kg - stock_actuel)
        montant_cmd   = qte_commander * m["prix_unit"]
        
        # Date recommandée de commande (tenir compte délai livraison)
        date_cmd = datetime.now() + timedelta(days=max(0, delai_livraison - int(couverture_mois*30)))
        
        # Urgence
        if couverture_mois < 1:      urgence = "🔴 Commander maintenant"
        elif couverture_mois < 2:    urgence = "🟡 Commander bientôt"
        else:                        urgence = "🟢 Stock suffisant"
        ...
        plan_rows.append({
            ...
            "Couverture":      round(couverture_mois, 1),
            "Besoin prévu":    round(besoin_kg),
            "Qté à commander": round(qte_commander),
            "Montant cmd":     round(montant_cmd),
            "Prix unitaire":   m["prix_unit"],
            "Date cmd recommandée": date_cmd.strftime("%d/%m/%Y"),
            "Urgence":         urgence,
            "Unité":           m["unite"],
        })

    plan_df = pd.DataFrame(plan_rows)
    budget_total_cmd = plan_df["Montant cmd"].sum()

    # ── KPIs ───────────────────────────────────────────────────────
    k1,k2,k3,k4,k5 = st.columns(5)
    with k1: kpi("💰","Budget Matières Prévu", fmt(budget_besoin), f"Sur {horizon_achat} mois","orange")
    with k2: kpi("🛒","Montant à Commander",   fmt(budget_total_cmd), "Commandes planifiées","bleu")
    with k3:
        nb_urgent = (plan_df["Urgence"].str.contains("maintenant")).sum()
        kpi("🔴","Urgences",str(nb_urgent),"Commander maintenant","rouge" if nb_urgent>0 else "vert")
    with k4:
        nb_bientot = (plan_df["Urgence"].str.contains("bientôt")).sum()
        kpi("🟡","À planifier",str(nb_bientot),"Commander bientôt","orange")
    with k5:
        ca_prev_ht = prev_ca_total/1.18
        kpi("📊","CA Prévu HT",fmt(ca_prev_ht),f"Base de calcul ({horizon_achat} mois)","violet")

    st.markdown("<br>", unsafe_allow_html=True)

    # ── Graphique 1 : Budget matières mensuel (CA prévu × taux) ──
    section("📈  CA PRÉVU & BUDGET MATIÈRES — Mois par mois")
    comment(
        f"La courbe bleue = CA prévu. Les barres orange = budget matières estimé ({taux_matieres_ca}% du CA HT). "
        "Ces barres représentent le montant à prévoir pour les achats chaque mois."
    )
    fig_ca = go.Figure()
    fig_ca.add_trace(go.Scatter(
        x=prev_ca_df["Période"], y=prev_ca_df["CA_Prévu"],
        name="CA Prévu (TTC)", mode="lines+markers",
        line=dict(color=C["bleu"],width=3),
        marker=dict(size=8,color=C["bleu"],line=dict(color="white",width=2)),
        hovertemplate="<b>%{x}</b><br>CA prévu : %{y:,.0f} FCFA<extra></extra>"))
    budget_mois = prev_ca_df["CA_Prévu"]/1.18*(taux_matieres_ca/100)
    fig_ca.add_trace(go.Bar(
        x=prev_ca_df["Période"], y=budget_mois,
        name=f"Budget Matières ({taux_matieres_ca}% CA HT)",
        marker_color=C["orange"], opacity=0.75,
        hovertemplate="<b>%{x}</b><br>Budget matières : %{y:,.0f} FCFA<extra></extra>"))
    fig_ca = excel_style(fig_ca,320)
    fig_ca.update_xaxes(tickangle=-20,tickfont=dict(size=10))
    fig_ca.update_yaxes(tickformat=",.0f",ticksuffix=" F")
    st.plotly_chart(fig_ca, use_container_width=True)

    st.markdown("<br>", unsafe_allow_html=True)

    # ── Graphique 2 : Répartition du budget par matière ──────────
    col_g1, col_g2 = st.columns(2)
    with col_g1:
        section("🥧  RÉPARTITION DU BUDGET PAR MATIÈRE")
        comment("Quelle matière première coûte le plus cher ? "
                "Le PE-HD et le PP représentent la majorité des achats.")
        fig_pie = go.Figure(go.Pie(
            labels=plan_df["Matière"],
            values=plan_df["Montant cmd"],
            hole=0.55,
            marker=dict(colors=PIE[:len(plan_df)],line=dict(color="white",width=2)),
            textinfo="label+percent",textfont=dict(size=10),
            hovertemplate="<b>%{label}</b><br>%{value:,.0f} FCFA (%{percent})<extra></extra>"))
        fig_pie.update_layout(paper_bgcolor="#FFFFFF",height=300,
            margin=dict(t=10,b=30,l=10,r=10),
            legend=dict(orientation="h",y=-0.1,xanchor="center",x=0.5,font=dict(size=9)))
        st.plotly_chart(fig_pie, use_container_width=True)

    with col_g2:
        section("⏱️  COUVERTURE STOCK ACTUELLE PAR MATIÈRE")
        comment("Combien de mois de production le stock actuel couvre-t-il ? "
                "🔴 < 1 mois = commander maintenant · 🟡 1–2 mois = planifier · 🟢 > 2 mois = OK.")
        clr_couv = [C["rouge"] if v<1 else (C["orange"] if v<2 else C["vert"])
                    for v in plan_df["Couverture"]]
        fig_cov = go.Figure(go.Bar(
            x=plan_df["Couverture"], y=plan_df["Matière"],
            orientation="h", marker_color=clr_couv,
            text=plan_df["Couverture"].apply(lambda v: f"{v:.1f} mois"),
            textposition="outside",textfont=dict(size=10),
            hovertemplate="<b>%{y}</b><br>Couverture : %{x:.1f} mois<extra></extra>"))
        fig_cov.add_vline(x=1,line_dash="dot",line_color=C["rouge"],line_width=1.5,
            annotation_text="Seuil urgent",annotation_font=dict(color=C["rouge"],size=9))
        fig_cov.add_vline(x=2,line_dash="dot",line_color=C["orange"],line_width=1.5,
            annotation_text="Seuil prudence",annotation_font=dict(color=C["orange"],size=9))
        fig_cov = excel_style(fig_cov,300,False)
        fig_cov.update_xaxes(ticksuffix=" mois",range=[0,max(plan_df["Couverture"].max()*1.3,3)])
        st.plotly_chart(fig_cov, use_container_width=True)

    st.markdown("<br>", unsafe_allow_html=True)

    # ── Alertes urgentes ──────────────────────────────────────────
    urgents = plan_df[plan_df["Urgence"].str.contains("maintenant")]
    bientots= plan_df[plan_df["Urgence"].str.contains("bientôt")]
    if len(urgents) > 0:
        section("🚨  COMMANDES URGENTES — À lancer maintenant")
        for _, r in urgents.iterrows():
            st.markdown(f"""
            <div class="alert-r">🔴<div>
              <b>COMMANDER MAINTENANT — {r["Matière"]}</b>
              <small> · Fournisseur : {r["Fournisseur"]}</small><br>
              <small>Stock actuel : <b>{r["Stock actuel"]:,} {r["Unité"]}</b>
              (couverture : <b>{r["Couverture"]:.1f} mois</b>) ·
              Qté à commander : <b>{r["Qté à commander"]:,.0f} {r["Unité"]}</b> ·
              Montant : <b>{fmt(r["Montant cmd"])}</b> ·
              Livraison sous : {r["Date cmd recommandée"]}</small>
            </div></div>""", unsafe_allow_html=True)

    if len(bientots) > 0:
        section("📅  COMMANDES À PLANIFIER — Dans les prochaines semaines")
        for _, r in bientots.iterrows():
            st.markdown(f"""
            <div class="alert-y">📅<div>
              <b>PLANIFIER — {r["Matière"]}</b>
              <small> · Fournisseur : {r["Fournisseur"]}</small><br>
              <small>Couverture : <b>{r["Couverture"]:.1f} mois</b> ·
              Qté recommandée : <b>{r["Qté à commander"]:,.0f} {r["Unité"]}</b> ·
              Montant : <b>{fmt(r["Montant cmd"])}</b> ·
              Date recommandée : {r["Date cmd recommandée"]}</small>
            </div></div>""", unsafe_allow_html=True)

    st.markdown("<br>", unsafe_allow_html=True)

    # ── Tableau plan de commande ──────────────────────────────────
    section("📋  PLAN DE COMMANDE COMPLET")
    comment("Ce tableau récapitule toutes les matières premières avec les quantités à commander "
            "et les dates recommandées. Exportez-le pour le transmettre au service achats.")
    plan_disp = plan_df.copy()
    plan_disp["Montant cmd"]   = plan_disp["Montant cmd"].apply(lambda v: f"{v:,.0f} FCFA")
    plan_disp["Prix unitaire"] = plan_disp["Prix unitaire"].apply(lambda v: f"{v:,} FCFA/{plan_disp['Unité'].iloc[0]}")
    plan_disp["Stock actuel"]  = plan_disp["Stock actuel"].apply(lambda v: f"{v:,}")
    plan_disp["Besoin prévu"]  = plan_disp["Besoin prévu"].apply(lambda v: f"{v:,}")
    plan_disp["Qté à commander"]= plan_disp["Qté à commander"].apply(lambda v: f"{v:,}")
    plan_disp["Couverture"]    = plan_disp["Couverture"].apply(lambda v: f"{v:.1f} mois")
    plan_disp = plan_disp.drop(columns=["Unité","Prix unitaire"])
    plan_disp = plan_disp.rename(columns={
        "Matière":"Matière Première","Besoin prévu":f"Besoin {horizon_achat}M (kg)",
        "Qté à commander":"Qté Commander (kg)","Montant cmd":"Montant",
        "Date cmd recommandée":"Date Commande","Couverture":"Couverture Stock"})
    st.dataframe(plan_disp, use_container_width=True, height=280)

    # ── Export plan d'achats ──────────────────────────────────────
    st.markdown("<br>", unsafe_allow_html=True)
    section("📥  EXPORTER LE PLAN D'ACHATS")
    col_ex1, col_ex2, _ = st.columns([1,1,2])
    with col_ex1:
        buf_achats = io.BytesIO()
        with pd.ExcelWriter(buf_achats, engine="openpyxl") as w:
            plan_df.to_excel(w, sheet_name="Plan Achats", index=False)
            prev_ca_df.to_excel(w, sheet_name="Prévisions CA", index=False)
        buf_achats.seek(0)
        st.download_button(
            "⬇️  Plan d'achats Excel",
            buf_achats,
            file_name=f"MULTIPACK_Achats_{datetime.now().strftime('%Y%m%d')}.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            use_container_width=True)
    with col_ex2:
        prevision_comment(
            f"Budget matières total pour les {horizon_achat} prochains mois : "
            f"<b>{fmt(budget_besoin)}</b>. "
            f"Commandes à lancer : <b>{fmt(budget_total_cmd)}</b> "
            f"(inclut le stock de sécurité de {stock_securite_pct}%). "
            f"Délai livraison moyen : {delai_livraison} jours.",
            "💡 Résumé planification"
        )


# ── PAGE IA & ANOMALIES ───────────────────────────────────────
elif page == "🤖  IA & Anomalies":

    st.markdown(f"""
    <div style="background:linear-gradient(135deg,#0F172A,#1E293B,{C['violet']});
                border-radius:14px;padding:22px 28px;margin-bottom:18px;
                border:1px solid rgba(124,58,237,0.3);">
      <div style="font-size:1.15rem;font-weight:800;color:white;margin-bottom:4px;">
        🤖 Intelligence Artificielle & Détection d'Anomalies</div>
      <div style="font-size:0.8rem;color:rgba(255,255,255,0.6);">
        Score de Santé · Isolation Forest · Random Forest · Conseiller IA MULTIPACK</div>
    </div>""", unsafe_allow_html=True)

    t_sc, t_an, t_pr, t_chat = st.tabs([
        "💊 Score de Santé",
        "🔍 Anomalies",
        "🎲 Prédiction Impayés",
        "💬 Conseiller IA",
    ...
    with t_sc:
        st.markdown("<br>", unsafe_allow_html=True)
        comment("Le <b>Score de Santé</b> résume en 0-100 la situation de MULTIPACK : "
                "recouvrement (35%), croissance (25%), stocks (20%), production (20%).","💊 Score de Santé Global")
        g_sc, sub_sc = score_sante(df_f, df_inv, df_prod)
        c_sc, c_det = st.columns([1,2])
        with c_sc:
            sc_col = C["vert"] if g_sc>=75 else (C["orange"] if g_sc>=50 else C["rouge"])
            sc_lbl = "Bonne santé" if g_sc>=75 else ("À surveiller" if g_sc>=50 else "Critique")
            st.markdown(f"""
            <div class="score-card">
              <div class="sc-lbl">Score de Santé Global</div>
              <div class="sc-num" style="color:{sc_col};">{g_sc:.0f}</div>
              <div style="font-size:0.7rem;color:#475569;">/100</div>
              <div style="margin:12px auto;width:80%;height:10px;background:#1E293B;border-radius:5px;">
                <div style="width:{g_sc:.0f}%;height:10px;background:{sc_col};border-radius:5px;"></div>
              </div>
              <div class="sc-sub">{sc_lbl}</div>
            </div>""", unsafe_allow_html=True)
        with c_det:
            section("📊  DÉCOMPOSITION DU SCORE")
            for dim, sc_v in sub_sc.items():
                pct = min(100, sc_v)
                c_d = C["vert"] if pct>=75 else (C["orange"] if pct>=50 else C["rouge"])
                st.markdown(f"""
                <div style="margin:8px 0;">
                  <div style="display:flex;justify-content:space-between;font-size:0.82rem;margin-bottom:5px;">
                    <b>{dim}</b><span style="font-weight:800;color:{c_d};">{pct:.0f}/100</span></div>
                  <div style="height:10px;background:{C['bordure']};border-radius:5px;overflow:hidden;">
                    <div style="height:10px;width:{pct:.0f}%;background:{c_d};border-radius:5px;"></div>
                  </div>
                </div>""", unsafe_allow_html=True)

    with t_an:
        st.markdown("<br>", unsafe_allow_html=True)
        comment("<b>Isolation Forest</b> détecte les factures inhabituelles (montant, période). "
                "Critique = investiguer. Modéré = surveiller. Faible = signal mineur.","🔍 Détection d'anomalies IA")
        with st.spinner("Analyse Isolation Forest…"):
            anom = detecter_anomalies(df_f)
        nb_cr = (anom["Niveau"]=="Critique").sum()
        nb_mo = (anom["Niveau"]=="Modere").sum()
        nb_fa = (anom["Niveau"]=="Faible").sum()
        ak1,ak2,ak3,ak4 = st.columns(4)
        with ak1: kpi("🔍","Anomalies",str(len(anom)),f"sur {len(df_f)} factures","violet")
        with ak2: kpi("🔴","Critiques",str(nb_cr),"Investiguer maintenant","rouge")
        with ak3: kpi("🟡","Modérées",str(nb_mo),"À surveiller","orange")
        with ak4: kpi("🟢","Faibles",str(nb_fa),"Signaux mineurs","vert")
        st.markdown("<br>", unsafe_allow_html=True)
        section("🚨  ANOMALIES DÉTECTÉES — Top 15")
        for _, row in anom.head(15).iterrows():
            niv = str(row.get("Niveau",""))
            clr = C["rouge"] if niv=="Critique" else (C["orange"] if niv=="Modere" else C["vert"])
            niv_txt = ("🔴 Critique" if niv=="Critique" else ("🟡 Modéré" if niv=="Modere" else "🟢 Faible"))
            st.markdown(f"""
            <div class="anomaly-row">
              <div>
                <div style="font-size:0.85rem;font-weight:700;">{row.get("N° FACTURE","—")}</div>
                <div style="font-size:0.77rem;color:{C["muted"]};">{row.get("CLIENT","—")} · {str(row.get("DATE",""))[:10]}</div>
                <div style="font-size:0.77rem;color:{C["muted"]};margin-top:2px;">{row.get("Raison","")}</div>
              </div>
              <div style="text-align:right;">
                <div style="font-size:0.82rem;font-weight:700;">{row.get("MONTANT TTC",0):,.0f} FCFA</div>
                <div style="font-size:0.78rem;font-weight:800;color:{clr};margin-top:4px;">{niv_txt}</div>
              </div>
            </div>""", unsafe_allow_html=True)

    with t_pr:
        st.markdown("<br>", unsafe_allow_html=True)
        comment("<b>Random Forest</b> estime la probabilité d'impayé par client/segment/zone/montant. "
                "Probabilité > 30% = proposer un acompte.","🎲 Prédiction risque impayé")
        with st.spinner("Entraînement Random Forest…"):
            df_pred, auc, imp_s = predire_impayes(df_f)
        pk1,pk2,pk3 = st.columns(3)
        with pk1: kpi("🎯","Score AUC",f"{auc:.3f}","Précision du modèle","vert" if auc>0.7 else "orange")
        with pk2:
            hr = int((df_pred.get("Proba_Imp",pd.Series([0]))>0.4).sum())
            kpi("⚠️","Haut Risque",str(hr),"Proba impayé > 40%","rouge")
        with pk3:
            kpi("💸","Créances",fmt(df_f[df_f["ETAT DE PAIEMENT"]=="Impayée"]["MONTANT TTC"].sum()),"À recouvrer","rouge")
        st.markdown("<br>", unsafe_allow_html=True)
        col_i1, col_i2 = st.columns(2)
        with col_i1:
            section("📊  IMPORTANCE DES VARIABLES")
            labels_m = {"log_m":"Montant","MOIS":"Mois","ANNEE":"Année","seg_e":"Segment","zon_e":"Zone"}
            imp_df = imp_s.sort_values(ascending=True).reset_index()
            imp_df.columns = ["Var","Val"]
            imp_df["Var"] = imp_df["Var"].map(labels_m)
            fig_im = go.Figure(go.Bar(x=imp_df["Val"],y=imp_df["Var"],orientation="h",
                marker_color=C["violet"],
                text=imp_df["Val"].apply(lambda v: f"{v:.1%}"),
                textposition="outside",textfont=dict(size=11),
                hovertemplate="<b>%{y}</b><br>%{x:.1%}<extra></extra>"))
            fig_im = excel_style(fig_im,260,False)
            fig_im.update_xaxes(tickformat=".0%")
            st.plotly_chart(fig_im, use_container_width=True)
        with col_i2:
            section("🎲  RISQUE PAR SEGMENT")
            if "Proba_Imp" in df_pred.columns:
                sr = df_pred.groupby("SEGMENT")["Proba_Imp"].mean().sort_values(ascending=True).reset_index()
                clr_s = [C["vert"] if v<0.2 else (C["orange"] if v<0.35 else C["rouge"]) for v in sr["Proba_Imp"]]
                fig_sr = go.Figure(go.Bar(x=sr["Proba_Imp"],y=sr["SEGMENT"],orientation="h",
                    marker_color=clr_s,
                    text=sr["Proba_Imp"].apply(lambda v: f"{v:.0%}"),
                    textposition="outside",textfont=dict(size=10),
                    hovertemplate="<b>%{y}</b><br>Risque : %{x:.0%}<extra></extra>"))
                fig_sr.add_vline(x=0.3,line_dash="dot",line_color=C["rouge"],
                    annotation_text="Seuil 30%",annotation_font=dict(color=C["rouge"],size=9))
                fig_sr = excel_style(fig_sr,260,False)
    ...
    with t_chat:
        ...
        .msg-ai .avatar {{
            ...
            border-radius: 50%;
            background: linear-gradient(135deg, {C['violet']}, {C['bleu']});
            display: flex; align-items: center; justify-content: center;
        ...
        .msg-ai .bubble b {{ color: {C['bleu']}; }}
        .msg-ai .bubble ul {{ margin: 6px 0 6px 16px; }}
        ...
        # ── Description ─────────────────────────────────────────
        st.markdown(f"""
        <div style="background:linear-gradient(135deg,{C['sidebar']},{C['violet']}33);
          ...
          <div style="display:flex;align-items:center;gap:12px;margin-bottom:10px;">
            ...
            <div>
              <div style="font-size:1rem;font-weight:800;color:white;">Conseiller IA MULTIPACK</div>
              <div style="font-size:0.72rem;color:#64748B;">Accès complet aux données · Analyse · Recommandations</div>
            </div>
          </div>
          <div style="font-size:0.82rem;color:#94A3B8;line-height:1.6;">
            Je connais <b style="color:white;">toutes vos données</b> : CA, clients, stocks, 
            production, paiements, marges. Posez-moi n'importe quelle question sur la performance 
            de MULTIPACK et je vous donnerai mon analyse avec des comparaisons sectorielles 
        ...
        def build_data_context():
            ...
            paye    = df_f[df_f["ETAT DE PAIEMENT"]=="Payée"]["MONTANT TTC"].sum()
            impaye  = ca - paye
            taux_r  = paye/ca*100 if ca>0 else 0
            nb_cl   = df_f["CLIENT"].nunique()
            nb_fact = len(df_f)
            panier  = df_f["MONTANT TTC"].mean()
            val_stk = df_inv["Valeur"].sum()
            nb_rupt = (df_inv["Statut"]=="Non disponible").sum()
            nb_faib = (df_inv["Statut"]=="Stock faible").sum()
            rend    = df_prod["Taux rendement"].mean()
            rebut   = df_prod["Taux rebut"].mean()

            # CA mensuel
            dm = df_f.copy()
            dm["P"] = dm["DATE"].dt.to_period("M").astype(str)
            ca_m = dm.groupby("P")["MONTANT TTC"].sum().sort_index()
            tendance_3m = (ca_m.iloc[-1]/ca_m.iloc[-3]*100 - 100) if len(ca_m)>=3 and ca_m.iloc[-3]>0 else 0

            # Top clients
            top_cl = df_f.groupby("CLIENT")["MONTANT TTC"].sum().sort_values(ascending=False).head(5)
            # Top segments
            top_seg = df_f.groupby("SEGMENT")["MONTANT TTC"].sum().sort_values(ascending=False)
            # Impayés top
            top_imp = df_f[df_f["ETAT DE PAIEMENT"]=="Impayée"].groupby("CLIENT")["MONTANT TTC"].sum().sort_values(ascending=False).head(5)
            # Marge estimée
            taux_mat = 0.42
            marge_brute = ca_ht * (1 - taux_mat)
            charges_fixes = 5_300_000
            ebitda_mois = (marge_brute/36 - charges_fixes)
            ebitda_pct = ebitda_mois / (ca_ht/36) * 100 if ca_ht>0 else 0

            ctx = f"""
Tu es le Conseiller IA officiel de MULTIPACK SA, fabricant de plastiques et emballages
basé à Yopougon, Abidjan (Côte d'Ivoire). Tu as accès à TOUTES les données de l'entreprise.
Tu parles en français, tu es direct, analytique, et tu compares toujours avec des benchmarks sectoriels.
Tu formules des recommandations concrètes, chiffrées et actionnables.

== DONNÉES RÉELLES MULTIPACK SA (période analysée) ==

CHIFFRE D'AFFAIRES :
- CA Total TTC : {ca:,.0f} FCFA
- CA HT : {ca_ht:,.0f} FCFA
- CA Encaissé : {paye:,.0f} FCFA ({taux_r:.1f}%)
- Créances Impayées : {impaye:,.0f} FCFA
- Taux de recouvrement : {taux_r:.1f}% (benchmark secteur CI : 78-85%)
- Nombre de factures : {nb_fact}
- Panier moyen : {panier:,.0f} FCFA / facture
- Tendance CA (3 derniers mois) : {tendance_3m:+.1f}%

CLIENTS :
- Nombre de clients actifs : {nb_cl}
- Top 5 clients par CA : {", ".join([f"{c} ({v/1e6:.1f}M FCFA)" for c,v in top_cl.items()])}
- Top 5 impayés : {", ".join([f"{c} ({v/1e6:.1f}M FCFA)" for c,v in top_imp.items()]) if len(top_imp)>0 else "Aucun"}

SEGMENTS :
{chr(10).join([f"- {s}: {v/1e6:.1f}M FCFA ({v/ca*100:.1f}%)" for s,v in top_seg.items()])}

STOCKS :
- Valeur totale du stock : {val_stk:,.0f} FCFA
- Références en rupture : {nb_rupt} (benchmark acceptable : 0)
- Références stock faible : {nb_faib}
- Rotation stock estimée : {df_sor["Quantité"].sum()/max(df_inv["Stock final"].sum(),1):.1f}x

PRODUCTION :
- Rendement moyen machines : {rend:.1f}% (objectif interne : 85%, benchmark CI industrie plastique : 80-88%)
- Taux de rebut moyen : {rebut:.2f}% (benchmark < 3%)
- Nombre de machines : 5 lignes (Presse Injection, Soufflage, Extrudeuse A&B, Thermoformeuse)

RENTABILITÉ ESTIMÉE :
- Taux marge brute estimé : {(1-taux_mat)*100:.0f}% du CA HT
- EBITDA mensuel estimé : {ebitda_mois:,.0f} FCFA ({ebitda_pct:.1f}% du CA HT mensuel)
- Charges fixes mensuelles estimées : 5 300 000 FCFA

BENCHMARKS SECTORIELS (industrie plastique Afrique de l'Ouest) :
- Taux recouvrement moyen : 78-85%
- Marge brute typique : 40-55%
- EBITDA typique : 12-20%
- Rendement machines : 80-88%
- Taux rebut acceptable : 1-3%
- Délai paiement moyen : 45-60 jours

== INSTRUCTIONS ==
- Réponds toujours en français
- Cite les chiffres réels de MULTIPACK dans chaque réponse
- Compare systématiquement avec les benchmarks sectoriels
- Donne des recommandations concrètes et chiffrées quand c'est pertinent
- Structure tes réponses avec des points clairs si la réponse est longue
- Sois direct et précis, évite le jargon inutile
- Quand tu identifies un problème, explique l'impact et propose une solution
"""
            return ctx

        # ── Suggestions de questions ─────────────────────────────
        SUGGESTIONS = [
            "📊 Analyse globale de MULTIPACK",
            "💰 Comment améliorer le CA ?",
            "⚠️ Quels sont les risques principaux ?",
            "👥 Quels clients prioriser ?",
            "💸 Pourquoi les impayés sont élevés ?",
            "🏭 Comment améliorer la production ?",
            "📦 Problèmes de stock ?",
            "🔮 Quelles prévisions pour l'année ?",
            "💹 Quelle est notre rentabilité ?",
            "🆚 Comment se compare-t-on au secteur ?",
        ]
            ...
            with sugg_cols[i]:
                if st.button(sugg, key=f"sugg_{i}", use_container_width=True):
                    st.session_state["chat_messages"].append({
                        "role": "user",
                        "content": sugg.split(" ",1)[1] if " " in sugg else sugg
                    })
                    st.session_state["trigger_response"] = True
                    st.rerun()
        sugg_cols2 = st.columns(5)
        for i, sugg in enumerate(SUGGESTIONS[5:]):
            with sugg_cols2[i]:
                if st.button(sugg, key=f"sugg2_{i}", use_container_width=True):
                    st.session_state["chat_messages"].append({
                        "role": "user",
                        "content": sugg.split(" ",1)[1] if " " in sugg else sugg
                    })
        ...
        if msgs_html:
            ...
            st.markdown(f"""
            <div class="chat-container" style="text-align:center;padding:40px 20px;color:{C['muted']};">
              <div style="font-size:2.5rem;margin-bottom:12px;">🤖</div>
              <div style="font-size:0.9rem;font-weight:600;">Bonjour ! Je suis votre Conseiller IA MULTIPACK.</div>
              <div style="font-size:0.8rem;margin-top:6px;">Posez-moi une question ou cliquez sur une suggestion ci-dessus.</div>
            </div>""", unsafe_allow_html=True)

        # ── Input utilisateur + bouton effacer ───────────────────
        col_inp, col_btn, col_clear = st.columns([6, 1.2, 1])
        with col_inp:
            user_input = st.text_input(
                "Message",
                placeholder="Ex: Quels sont nos clients les plus risqués ? Comment améliorer notre trésorerie ?",
                label_visibility="collapsed",
                key="chat_input",
            )
        with col_btn:
        ...
        if (send and user_input.strip()) or trigger:
            ...
            system_prompt = build_data_context()
            api_messages  = []
            ...
            if not api_key:
                st.warning("🔑 Entrez votre clé API Anthropic dans la barre latérale pour activer le chatbot.")
            else:
                with st.spinner("🤖 Analyse en cours…"):
                    try:
                        ...
                        if e.code == 401:
                            assistant_reply = "❌ Clé API invalide. Vérifiez votre clé dans la barre latérale."
                        elif e.code == 429:
                            assistant_reply = "⏳ Limite de requêtes atteinte. Attendez quelques secondes."
                        else:
                            assistant_reply = f"⚠️ Erreur API ({e.code}). Détail : {body[:200]}"
                    except Exception as e:
                        assistant_reply = (
                            f"⚠️ Impossible de joindre l'API. "
...
# ══════════════════════════════════════════════════════════════
# PAGE PILOTAGE RH — Données générées · Corrélations · Impact
# ══════════════════════════════════════════════════════════════
elif page == "👷  Pilotage RH":

    import hashlib

    # ── Génération des données RH (36 mois, cohérentes avec prod) ──
    @st.cache_data(show_spinner=False)
    def generer_donnees_rh():
        np.random.seed(99)
        mois_list = pd.date_range("2022-01-01", periods=36, freq="MS")

        # Profils des employés
        POSTES = {
            "Opérateur Machine":     {"base": 185_000, "cat": "Production"},
            "Chef d'Équipe":          {"base": 290_000, "cat": "Production"},
            "Technicien Maintenance": {"base": 330_000, "cat": "Technique"},
            "Agent Qualité":          {"base": 250_000, "cat": "Qualité"},
            "Commercial Terrain":     {"base": 320_000, "cat": "Commercial"},
            "Assistant Admin":        {"base": 160_000, "cat": "Administratif"},
            "Magasinier":             {"base": 195_000, "cat": "Logistique"},
            "Comptable":              {"base": 280_000, "cat": "Finance"},
        }

        # Effectifs mensuels simulés
        rh_mois = []
        for i, mois in enumerate(mois_list):
            # Tendance légère croissance effectifs
            cdi_prod   = int(np.clip(12 + i*0.1 + np.random.normal(0,1), 10, 16))
            journ_prod  = int(np.clip(8  - i*0.05 + np.random.normal(0,2), 3, 14))
            quint_prod  = int(np.clip(4  + np.random.normal(0,1), 2, 7))
            cdi_autres  = int(np.clip(8  + i*0.05 + np.random.normal(0,0.5), 7, 11))
            nb_comm     = int(np.clip(4  + (1 if i>18 else 0) + np.random.normal(0,0.5), 3, 6))

            total_prod  = cdi_prod + journ_prod + quint_prod
            total_all   = total_prod + cdi_autres + nb_comm

            # Masse salariale
            ms_cdi    = cdi_prod   * 185_000 * 1.35 + cdi_autres * 250_000 * 1.35
            ms_journ  = journ_prod * 185_000 / 22 * 26 * 1.15  # journalier = légèrement + cher/unité
            ms_quint  = quint_prod * 185_000 * 0.5 * 1.20
            ms_comm   = nb_comm    * 320_000 * 1.35
            ms_total  = ms_cdi + ms_journ + ms_quint + ms_comm

            # Absentéisme (corrélé avec % journaliers)
            pct_journ   = journ_prod / max(total_prod, 1)
            absent_taux = np.clip(0.04 + pct_journ * 0.12 + np.random.normal(0, 0.01), 0.01, 0.18)
            absent_jours= int(total_all * absent_taux * 22)

            # Turnover (annualisé)
            departs  = int(np.clip(np.random.poisson(1.2), 0, 4))
            arrivees = int(np.clip(np.random.poisson(1.3), 0, 4))

            # Formation
            nb_formations   = int(np.clip(np.random.poisson(2), 0, 6))
            heures_formation= nb_formations * int(np.random.uniform(4, 16))
            cout_formation  = heures_formation * 15_000

            # Ancienneté moyenne production (augmente avec moins de turnover)
            anciennete_moy = np.clip(14 - i * 0.1 + departs * 2 - arrivees + np.random.normal(0, 1), 6, 36)

            rh_mois.append({
                "Mois":               mois,
                "Mois_Str":           mois.strftime("%Y-%m"),
                "Mois_Label":         mois.strftime("%b %Y"),
                "CDI_Production":     cdi_prod,
                "Journaliers":        journ_prod,
                "Quinzainiers":       quint_prod,
                "CDI_Autres":         cdi_autres,
                "Commerciaux":        nb_comm,
                "Total_Prod":         total_prod,
                "Total_Effectif":     total_all,
                "Pct_Journaliers":    round(pct_journ * 100, 1),
                "Masse_Salariale":    round(ms_total),
                "Cout_MO_Unite":      round(ms_total / max(total_prod * 400 * 22, 1), 1),
                "Absenteisme_Taux":   round(absent_taux * 100, 1),
                "Absences_Jours":     absent_jours,
                "Departs":            departs,
                "Arrivees":           arrivees,
                "Nb_Formations":      nb_formations,
                "Heures_Formation":   heures_formation,
                "Cout_Formation":     cout_formation,
                "Anciennete_Moy_Mois":round(anciennete_moy, 1),
            })

        df_rh = pd.DataFrame(rh_mois)

        # ── Corrélation avec prod (mêmes seeds pour cohérence) ──
        np.random.seed(42)
        SAISON = [0.75,0.70,0.80,0.85,0.90,0.97,0.88,1.12,1.22,1.30,1.45,1.38]
        prod_mois = []
        for i, row in df_rh.iterrows():
            m  = row["Mois"].month
            co = SAISON[m-1]
            # Rebut corrélé au % journaliers et absentéisme
            rebut_base = 2.0
            rebut_journ= row["Pct_Journaliers"] * 0.04   # +0.04% par % de journaliers
            rebut_absent= row["Absenteisme_Taux"] * 0.08
            rebut_anc  = max(0, (18 - row["Anciennete_Moy_Mois"]) * 0.05)
            taux_rebut  = np.clip(rebut_base + rebut_journ + rebut_absent + rebut_anc + np.random.normal(0, 0.2), 0.8, 6.5)
            # Rendement corrélé à ancienneté et formations
            rend_base   = 82.0
            rend_anc    = row["Anciennete_Moy_Mois"] * 0.2
            rend_form   = row["Heures_Formation"] * 0.05
            rend_absent = -row["Absenteisme_Taux"] * 0.4
            taux_rend   = np.clip(rend_base + rend_anc + rend_form + rend_absent + np.random.normal(0, 1.5), 72, 97)
            # Production totale
            prod_plan   = int(750_000 * co)
            prod_reel   = int(prod_plan * taux_rend / 100)
            rebuts      = int(prod_reel * taux_rebut / 100)
            # CA corrélé aux commerciaux
            ca_par_comm = 8_500_000 + row["Commerciaux"] * 1_200_000
            ca_mois     = ca_par_comm * co * np.random.uniform(0.85, 1.15)

            prod_mois.append({
                "Mois_Str":         row["Mois_Str"],
                "Taux_Rebut":       round(taux_rebut, 2),
                "Taux_Rendement":   round(taux_rend, 1),
                "Production_Reelle":prod_reel,
                "Rebuts":           rebuts,
                "CA_Mensuel":       round(ca_mois),
            })

        df_prod_rh = pd.DataFrame(prod_mois)
        df_merge   = df_rh.merge(df_prod_rh, on="Mois_Str")
        return df_rh, df_prod_rh, df_merge

    with st.spinner("Chargement du module Pilotage RH…"):
        df_rh, df_prod_rh, df_m = generer_donnees_rh()
    ...
    <div style="background:linear-gradient(135deg,#0F172A,#1E293B,{C['vert']});
                ...
                border:1px solid rgba(16,185,129,0.3);">
      <div style="font-size:1.15rem;font-weight:800;color:white;margin-bottom:4px;">
        👷 Pilotage RH — Impact du Personnel sur la Production et les Ventes</div>
      <div style="font-size:0.8rem;color:rgba(255,255,255,0.65);">
        Corrélations CDI/Journaliers · Absentéisme · Formations · Masse Salariale · Rendement</div>
    </div>""", unsafe_allow_html=True)

    comment(
        "Ce module analyse l'impact de vos choix RH sur la rentabilité. "
        "Il répond aux questions clés : <b>Est-ce que prendre plus de journaliers augmente les rebuts ?</b> "
        "<b>Les formations améliorent-elles le rendement ?</b> "
        "<b>Un commercial de plus, ça rapporte combien ?</b> "
        "Toutes les corrélations sont calculées automatiquement sur 36 mois de données.",
    ...
    with t1:
        ...
        last = df_rh.iloc[-1]
        prev = df_rh.iloc[-4]  # vs 3 mois avant

        k1,k2,k3,k4,k5,k6 = st.columns(6)
        with k1: kpi("👥","Effectif Total", str(int(last["Total_Effectif"])),
                     f"dont {int(last['Total_Prod'])} production","bleu")
        with k2:
            pj = last["Pct_Journaliers"]
            kpi("📋","% Journaliers", f"{pj:.0f}%",
                f"{'⚠️ Élevé' if pj>40 else '✅ Normal'}","orange" if pj>40 else "vert")
        with k3:
            ab = last["Absenteisme_Taux"]
            kpi("🏥","Absentéisme", f"{ab:.1f}%",
                "Objectif < 5%","rouge" if ab>8 else ("orange" if ab>5 else "vert"))
        with k4:
            turn = (df_rh["Departs"].tail(12).sum() / df_rh["Total_Effectif"].tail(12).mean() * 100)
            kpi("🚪","Turnover Annuel", f"{turn:.0f}%",
                "Objectif < 15%","rouge" if turn>20 else ("orange" if turn>15 else "vert"))
        with k5: kpi("🎓","Formations/Mois", str(int(last["Nb_Formations"])),
                     f"{int(last['Heures_Formation'])}h ce mois","vert")
        with k6: kpi("⏱️","Ancienneté Moy.", f"{last['Anciennete_Moy_Mois']:.0f} mois",
                     "Production","bleu")

        st.markdown("<br>", unsafe_allow_html=True)

        # Graphique 1 — Composition effectifs dans le temps
        section("👥  ÉVOLUTION DE LA COMPOSITION DES EFFECTIFS")
        comment("Les barres montrent comment vos effectifs se répartissent chaque mois. "
                "Un ratio journaliers élevé (barres orange) = plus de flexibilité mais moins d'expertise. "
                "Surveillez la tendance : augmente-t-elle ou se stabilise-t-elle ?")
        fig_eff = go.Figure()
        fig_eff.add_trace(go.Bar(x=df_rh["Mois_Label"], y=df_rh["CDI_Production"],
            name="CDI Production", marker_color=C["bleu"],
            hovertemplate="<b>%{x}</b><br>CDI Prod : %{y}<extra></extra>"))
        fig_eff.add_trace(go.Bar(x=df_rh["Mois_Label"], y=df_rh["Journaliers"],
            name="Journaliers", marker_color=C["orange"],
            hovertemplate="<b>%{x}</b><br>Journaliers : %{y}<extra></extra>"))
        fig_eff.add_trace(go.Bar(x=df_rh["Mois_Label"], y=df_rh["Quinzainiers"],
            name="Quinzainiers", marker_color=C["amber"] if "amber" in C else C["or"],
            hovertemplate="<b>%{x}</b><br>Quinzainiers : %{y}<extra></extra>"))
        fig_eff.add_trace(go.Bar(x=df_rh["Mois_Label"], y=df_rh["CDI_Autres"],
            name="CDI Autres services", marker_color=C["violet"], opacity=0.7,
            hovertemplate="<b>%{x}</b><br>CDI Autres : %{y}<extra></extra>"))
        fig_eff.add_trace(go.Bar(x=df_rh["Mois_Label"], y=df_rh["Commerciaux"],
            name="Commerciaux", marker_color=C["vert"],
            hovertemplate="<b>%{x}</b><br>Commerciaux : %{y}<extra></extra>"))
        fig_eff.update_layout(barmode="stack")
        fig_eff = excel_style(fig_eff, 360)
        fig_eff.update_xaxes(tickangle=-35, tickfont=dict(size=8))
        st.plotly_chart(fig_eff, use_container_width=True)

        st.markdown("<br>", unsafe_allow_html=True)
        col_a, col_b = st.columns(2)
        with col_a:
            section("🏥  TAUX D'ABSENTÉISME MENSUEL")
            comment("Au-dessus de 5% = signal d'alerte. Corrigé avec le % journaliers.")
            clr_ab = [C["vert"] if v<5 else (C["orange"] if v<8 else C["rouge"])
                      for v in df_m["Absenteisme_Taux"]]
            fig_ab = go.Figure(go.Bar(
                x=df_m["Mois_Label"], y=df_m["Absenteisme_Taux"],
                marker_color=clr_ab,
                hovertemplate="<b>%{x}</b><br>Absent. : %{y:.1f}%<extra></extra>"))
            fig_ab.add_hline(y=5, line_dash="dot", line_color=C["orange"], line_width=1.5,
                annotation_text="Seuil 5%", annotation_font=dict(color=C["orange"],size=9))
            fig_ab = excel_style(fig_ab, 280, False)
            fig_ab.update_xaxes(tickangle=-35, tickfont=dict(size=8))
            fig_ab.update_yaxes(ticksuffix="%")
            st.plotly_chart(fig_ab, use_container_width=True)

        with col_b:
            section("🎓  FORMATIONS — Heures par mois")
            comment("Chaque heure de formation investie améliore le rendement. "
                    "Corrélation mesurée dans l'onglet suivant.")
            fig_form = go.Figure(go.Bar(
                x=df_m["Mois_Label"], y=df_m["Heures_Formation"],
                marker_color=C["violet"], opacity=0.85,
                hovertemplate="<b>%{x}</b><br>%{y}h de formation<extra></extra>"))
            fig_form = excel_style(fig_form, 280, False)
            fig_form.update_xaxes(tickangle=-35, tickfont=dict(size=8))
            fig_form.update_yaxes(ticksuffix=" h")
            st.plotly_chart(fig_form, use_container_width=True)

    # ════════════════════════════════════════════════════════════
    # TAB 2 — CORRÉLATIONS CLÉS
    # ════════════════════════════════════════════════════════════
    with t2:
        st.markdown("<br>", unsafe_allow_html=True)
        comment(
            "Une corrélation mesure la force du lien entre deux variables. "
            "<b>+1</b> = lien positif parfait. <b>-1</b> = lien inverse parfait. "
            "<b>0</b> = aucun lien. Au-delà de ±0.5 = corrélation significative à surveiller.",
        ...
        def corr_info(x, y, label_x, label_y):
        ...
        correls = {
            ("% Journaliers", "Taux de Rebut"):
                corr_info(df_m["Pct_Journaliers"].values, df_m["Taux_Rebut"].values, "% Journaliers", "Rebut"),
            ("% Journaliers", "Taux de Rendement"):
                corr_info(df_m["Pct_Journaliers"].values, df_m["Taux_Rendement"].values, "% Journaliers", "Rendement"),
            ("Absentéisme %", "Taux de Rebut"):
                corr_info(df_m["Absenteisme_Taux"].values, df_m["Taux_Rebut"].values, "Absentéisme", "Rebut"),
            ("Absentéisme %", "Taux de Rendement"):
                corr_info(df_m["Absenteisme_Taux"].values, df_m["Taux_Rendement"].values, "Absentéisme", "Rendement"),
            ("Heures Formation", "Taux de Rendement"):
                corr_info(df_m["Heures_Formation"].values, df_m["Taux_Rendement"].values, "Formations", "Rendement"),
            ("Ancienneté Moy.", "Taux de Rendement"):
                corr_info(df_m["Anciennete_Moy_Mois"].values, df_m["Taux_Rendement"].values, "Ancienneté", "Rendement"),
            ("Nb Commerciaux", "CA Mensuel"):
                corr_info(df_m["Commerciaux"].values.astype(float), df_m["CA_Mensuel"].values, "Commerciaux", "CA"),
            ("Ancienneté Moy.", "Taux de Rebut"):
                corr_info(df_m["Anciennete_Moy_Mois"].values, df_m["Taux_Rebut"].values, "Ancienneté", "Rebut"),
        }

        # Affichage des corrélations
        section("🔗  TABLEAU DES CORRÉLATIONS RH → PERFORMANCE")

        for (var1, var2), (r, p) in correls.items():
            abs_r = abs(r)
            if abs_r >= 0.6:      force = "Forte"; col_f = C["rouge"] if r > 0 else C["vert"]
            elif abs_r >= 0.35:   force = "Modérée"; col_f = C["orange"]
            else:                 force = "Faible"; col_f = C["muted"]

            dir_txt = "↑ Positive" if r > 0 else "↓ Inverse"
            dir_col = C["rouge"] if r > 0 else C["vert"]

            # Signification métier
            interp = {
                ("% Journaliers", "Taux de Rebut"):
                    f"+10% de journaliers = +{abs(r)*10*0.4:.1f}% de rebut estimé → perte matière directe",
                ("% Journaliers", "Taux de Rendement"):
                    f"Plus de journaliers = moins bon rendement · Impact : {abs(r)*10:.1f}% de rendement par 10pts",
                ("Absentéisme %", "Taux de Rebut"):
                    f"Absentéisme élevé → rebuts en hausse · Lien fort avec le remplacement à la volée",
                ("Absentéisme %", "Taux de Rendement"):
                    f"Chaque +1% d'absentéisme = -{abs(r):.2f} pts de rendement estimé",
                ("Heures Formation", "Taux de Rendement"):
                    f"Formation = investissement ROI mesurable · +10h = +{abs(r)*10*0.05:.1f}% rendement",
                ("Ancienneté Moy.", "Taux de Rendement"):
                    f"Opérateurs expérimentés = meilleur rendement · Chaque mois d'ancienneté compte",
                ("Nb Commerciaux", "CA Mensuel"):
                    f"1 commercial de plus = +{abs(r)*1_200_000/abs(r+0.001):,.0f} FCFA/mois estimé",
                ("Ancienneté Moy.", "Taux de Rebut"):
                    f"Plus anciens = moins de rebuts · L'expérience réduit les erreurs de réglage",
            }.get((var1, var2), "Corrélation calculée sur 36 mois de données")

            # Barre de corrélation visuelle
            bar_w  = int(abs_r * 100)
            bar_col= C["rouge"] if r > 0 and "Rebut" in var2 else (
                     C["vert"] if (r < 0 and "Rebut" in var2) or (r > 0 and "Rendement" in var2) else
                     C["bleu"])

            st.markdown(f"""
            <div style="background:{C['blanc']};border:1px solid {C['bordure']};
                        border-radius:12px;padding:14px 18px;margin:8px 0;
                        box-shadow:0 1px 4px rgba(0,0,0,0.04);">
              <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">
                <div>
                  <span style="font-size:0.88rem;font-weight:700;color:{C['texte']};">
                    {var1}</span>
                  <span style="font-size:0.82rem;color:{C['muted']};margin:0 6px;">→</span>
                  <span style="font-size:0.88rem;font-weight:700;color:{C['texte']};">{var2}</span>
                </div>
                <div style="display:flex;gap:12px;align-items:center;">
                  <span style="font-size:0.75rem;font-weight:700;color:{dir_col};
                               background:rgba(0,0,0,0.04);border-radius:20px;padding:2px 10px;">
                    {dir_txt}</span>
                  <span style="font-size:1rem;font-weight:900;color:{col_f};">r = {r:+.3f}</span>
                  <span style="font-size:0.72rem;color:{col_f};font-weight:700;
                               background:{col_f}22;border-radius:20px;padding:2px 10px;">
                    {force}</span>
                </div>
              </div>
              <div style="height:8px;background:{C['bordure']};border-radius:4px;overflow:hidden;margin-bottom:8px;">
                <div style="height:8px;width:{bar_w}%;background:{bar_col};border-radius:4px;transition:width 0.5s;"></div>
              </div>
              <div style="font-size:0.8rem;color:{C['muted']};font-style:italic;">{interp}</div>
            </div>""", unsafe_allow_html=True)

        st.markdown("<br>", unsafe_allow_html=True)

        # Scatter plots des 3 corrélations les plus fortes
        section("📊  VISUALISATION — Les 3 corrélations les plus significatives")

        sc1, sc2, sc3 = st.columns(3)
        scatter_data = [
            (sc1, "Pct_Journaliers", "Taux_Rebut",
             "% Journaliers vs Taux Rebut", C["rouge"],
             "Plus de journaliers → plus de rebuts"),
            (sc2, "Anciennete_Moy_Mois", "Taux_Rendement",
             "Ancienneté vs Rendement", C["vert"],
             "Opérateurs expérimentés → meilleur rendement"),
            (sc3, "Heures_Formation", "Taux_Rendement",
             "Formations vs Rendement", C["violet"],
             "Formation → amélioration mesurable du rendement"),
        ]
        for col_st, x_col, y_col, title, color, note in scatter_data:
            with col_st:
                r_val, _ = corr_info(df_m[x_col].values, df_m[y_col].values, x_col, y_col)
                fig_sc = go.Figure()
                fig_sc.add_trace(go.Scatter(
                    x=df_m[x_col], y=df_m[y_col],
                    mode="markers",
                    marker=dict(size=8, color=color, opacity=0.75,
                                line=dict(color="white",width=1.5)),
                    hovertemplate=f"<b>%{{x:.1f}}</b> → %{{y:.1f}}<extra></extra>"))
                # Ligne de tendance
                z = np.polyfit(df_m[x_col].dropna(), df_m[y_col].dropna(), 1)
                p = np.poly1d(z)
                x_line = np.linspace(df_m[x_col].min(), df_m[x_col].max(), 50)
                fig_sc.add_trace(go.Scatter(
                    x=x_line, y=p(x_line), mode="lines",
                    line=dict(color=color, width=2, dash="dash"),
                    showlegend=False))
                fig_sc = excel_style(fig_sc, 260, False)
                fig_sc.update_layout(
                    title=dict(text=f"<b>r = {r_val:+.3f}</b>",
                               font=dict(size=11,color=color),x=0.5),
                    margin=dict(t=40,b=40,l=10,r=10))
                st.plotly_chart(fig_sc, use_container_width=True)
                st.markdown(f"""<div style="font-size:0.75rem;color:{C['muted']};
                    text-align:center;margin-top:-8px;">{note}</div>""",
                    unsafe_allow_html=True)

    # ════════════════════════════════════════════════════════════
    # TAB 3 — MASSE SALARIALE
    # ════════════════════════════════════════════════════════════
    with t3:
        st.markdown("<br>", unsafe_allow_html=True)
        comment(
            "La masse salariale est le 1er poste de charges de MULTIPACK. "
            "Ce module analyse son évolution, sa composition et son rapport au CA. "
            "Un ratio Masse Salariale / CA au-dessus de 25% mérite attention.",
            "💰 Comment lire la masse salariale ?"
        )

        ms_total = df_m["Masse_Salariale"].sum()
        ms_moy   = df_m["Masse_Salariale"].mean()
        ms_last  = df_m["Masse_Salariale"].iloc[-1]
        ms_prev  = df_m["Masse_Salariale"].iloc[-4]
        ratio_ms_ca = df_m["Masse_Salariale"].mean() / df_m["CA_Mensuel"].mean() * 100

        mk1,mk2,mk3,mk4 = st.columns(4)
        with mk1: kpi("💰","MS Mensuelle Moy.", fmt(ms_moy), "36 mois","bleu")
        with mk2: kpi("📈","MS Dernier Mois",   fmt(ms_last),
                      f"{'▲' if ms_last>ms_prev else '▼'} vs 3 mois avant",
                      "rouge" if ms_last > ms_prev*1.05 else "vert")
        with mk3: kpi("📊","MS / CA (%)", f"{ratio_ms_ca:.1f}%",
                      "Ratio masse salariale/CA","vert" if ratio_ms_ca<22 else ("orange" if ratio_ms_ca<28 else "rouge"))
        with mk4:
            cout_journ = df_m["Journaliers"].mean() * 185_000 / 22 * 26 * 1.15
            cout_cdi   = df_m["CDI_Production"].mean() * 185_000 * 1.35
            kpi("⚖️","Surcoût Journaliers", fmt(cout_journ - df_m["CDI_Production"].mean()*185_000*1.35/df_m["CDI_Production"].mean()*df_m["Journaliers"].mean()),
                "vs équivalent CDI/mois","orange")

        st.markdown("<br>", unsafe_allow_html=True)

        # Graphique masse salariale vs CA
        section("💰  MASSE SALARIALE VS CHIFFRE D'AFFAIRES")
        comment("La ligne bleue = masse salariale. Les barres vertes = CA mensuel. "
                "La distance entre les deux = votre marge avant autres charges. "
                "Si les barres rétrécissent = la masse salariale pèse de plus en plus.")
        fig_ms = go.Figure()
        fig_ms.add_trace(go.Bar(
            x=df_m["Mois_Label"], y=df_m["CA_Mensuel"],
            name="CA Mensuel", marker_color=C["vert"], opacity=0.6,
            hovertemplate="<b>%{x}</b><br>CA : %{y:,.0f} FCFA<extra></extra>"))
        fig_ms.add_trace(go.Scatter(
            x=df_m["Mois_Label"], y=df_m["Masse_Salariale"],
            name="Masse Salariale", mode="lines+markers",
            line=dict(color=C["rouge"], width=3),
            marker=dict(size=6, color=C["rouge"]),
            hovertemplate="<b>%{x}</b><br>MS : %{y:,.0f} FCFA<extra></extra>"))
        fig_ms = excel_style(fig_ms, 320)
        fig_ms.update_xaxes(tickangle=-35, tickfont=dict(size=8))
        fig_ms.update_yaxes(tickformat=",.0f", ticksuffix=" F")
        st.plotly_chart(fig_ms, use_container_width=True)

        st.markdown("<br>", unsafe_allow_html=True)
        col_ms1, col_ms2 = st.columns(2)
        with col_ms1:
            section("📊  RATIO MS/CA MENSUEL")
            comment("Doit rester sous 25%. Rouge = point d'attention.")
            ratio_mois = (df_m["Masse_Salariale"] / df_m["CA_Mensuel"] * 100).round(1)
            clr_r = [C["vert"] if v<22 else (C["orange"] if v<28 else C["rouge"]) for v in ratio_mois]
            fig_ratio = go.Figure(go.Bar(
                x=df_m["Mois_Label"], y=ratio_mois,
                marker_color=clr_r,
                hovertemplate="<b>%{x}</b><br>MS/CA : %{y:.1f}%<extra></extra>"))
            fig_ratio.add_hline(y=25, line_dash="dot", line_color=C["orange"],
                annotation_text="Seuil 25%", annotation_font=dict(color=C["orange"],size=9))
            fig_ratio = excel_style(fig_ratio, 280, False)
            fig_ratio.update_xaxes(tickangle=-35, tickfont=dict(size=8))
            fig_ratio.update_yaxes(ticksuffix="%")
            st.plotly_chart(fig_ratio, use_container_width=True)

        with col_ms2:
            section("⚖️  COÛT CDI vs JOURNALIER — Comparaison réelle")
            comment("Le journalier semble moins cher mais coûte plus par unité produite "
                    "à cause de la moindre productivité et des frais d'embauche répétés.")
            # Coût par unité produite
            df_m["Cout_Unit_CDI"]   = (df_m["CDI_Production"] * 185_000 * 1.35) / (df_m["CDI_Production"] * 400 * 22).clip(1)
            df_m["Cout_Unit_Journ"] = (df_m["Journaliers"] * 185_000 / 22 * 26 * 1.15) / (df_m["Journaliers"] * 380 * 22).clip(1)
            fig_cu = go.Figure()
            fig_cu.add_trace(go.Scatter(
                x=df_m["Mois_Label"], y=df_m["Cout_Unit_CDI"].round(2),
                name="Coût/unité CDI", mode="lines",
                line=dict(color=C["bleu"], width=2.5),
                hovertemplate="<b>%{x}</b><br>CDI : %{y:.2f} FCFA/u<extra></extra>"))
            fig_cu.add_trace(go.Scatter(
                x=df_m["Mois_Label"], y=df_m["Cout_Unit_Journ"].round(2),
                name="Coût/unité Journalier", mode="lines",
                line=dict(color=C["orange"], width=2.5, dash="dash"),
                hovertemplate="<b>%{x}</b><br>Journalier : %{y:.2f} FCFA/u<extra></extra>"))
            fig_cu = excel_style(fig_cu, 280)
            fig_cu.update_xaxes(tickangle=-35, tickfont=dict(size=8))
            fig_cu.update_yaxes(ticksuffix=" F/u")
            st.plotly_chart(fig_cu, use_container_width=True)

    # ════════════════════════════════════════════════════════════
    # TAB 4 — IMPACT COMMERCIAL
    # ════════════════════════════════════════════════════════════
    with t4:
        st.markdown("<br>", unsafe_allow_html=True)
        comment(
            "Combien rapporte un commercial supplémentaire ? "
            "Cette analyse calcule la corrélation entre le nombre de commerciaux et le CA, "
            "et estime le ROI d'un recrutement commercial.",
            "📈 Impact d'un commercial sur le CA"
        )

        # Corrélation commerciaux / CA
        r_comm, p_comm = corr_info(
            df_m["Commerciaux"].values.astype(float),
            df_m["CA_Mensuel"].values, "Comm", "CA")

        # CA moyen par commercial
        ca_par_comm_moy = df_m.groupby("Commerciaux")["CA_Mensuel"].mean()

        # Régression linéaire CA = a * nb_commerciaux + b
        from sklearn.linear_model import LinearRegression
        X_c = df_m[["Commerciaux"]].values
        y_c = df_m["CA_Mensuel"].values
        lr  = LinearRegression().fit(X_c, y_c)
        ca_par_comm_additionnel = lr.coef_[0]

        ck1,ck2,ck3,ck4 = st.columns(4)
        with ck1: kpi("🔗","Corrélation Comm./CA", f"{r_comm:+.3f}",
                      "Forte" if abs(r_comm)>0.6 else "Modérée","vert" if abs(r_comm)>0.5 else "orange")
        with ck2: kpi("💰","CA additionnel/comm.", fmt(ca_par_comm_additionnel),
                      "Par commercial supplémentaire","bleu")
        with ck3:
            salaire_comm = 320_000 * 1.35  # avec charges
            roi_comm = ca_par_comm_additionnel / salaire_comm
            kpi("📈","ROI d'un Recrutement",f"x{roi_comm:.1f}",
                "CA généré / coût salarial","vert" if roi_comm>3 else "orange")
        with ck4:
            marge_comm = ca_par_comm_additionnel * 0.58 * 0.15 - salaire_comm
            kpi("💹","Marge nette / comm.", fmt(marge_comm),
                "Après coût salarial (marge 15%)","vert" if marge_comm>0 else "rouge")

        st.markdown("<br>", unsafe_allow_html=True)

        section("📊  CA MENSUEL SELON LE NOMBRE DE COMMERCIAUX")
        comment("Chaque couleur = une période avec un nombre différent de commerciaux. "
                "La tendance montre clairement que plus de commerciaux = plus de CA.")
        fig_comm = go.Figure()
        for nb in sorted(df_m["Commerciaux"].unique()):
            d = df_m[df_m["Commerciaux"]==nb]
            fig_comm.add_trace(go.Scatter(
                x=d["Mois_Label"], y=d["CA_Mensuel"],
                mode="markers+lines", name=f"{nb} commerciaux",
                marker=dict(size=9, line=dict(color="white",width=1.5)),
                line=dict(width=2),
                hovertemplate=f"<b>{nb} comm.</b><br>%{{x}}<br>CA : %{{y:,.0f}} FCFA<extra></extra>"))
        fig_comm = excel_style(fig_comm, 320)
        fig_comm.update_xaxes(tickangle=-35, tickfont=dict(size=8))
        fig_comm.update_yaxes(tickformat=",.0f", ticksuffix=" F")
        st.plotly_chart(fig_comm, use_container_width=True)

        prevision_comment(
            f"Sur vos 36 mois de données, chaque commercial supplémentaire génère en moyenne "
            f"<b>{fmt(ca_par_comm_additionnel)}</b> de CA mensuel additionnel. "
            f"Avec un coût salarial total de {fmt(salaire_comm)}/mois, "
            f"le ROI est de <b>x{roi_comm:.1f}</b>. "
            f"Si votre taux de marge brute est de 15%, la marge nette générée est "
            f"<b>{fmt(marge_comm)}</b>/mois. "
            f"Un recrutement commercial s'autofinance en moins de 2 mois.",
            "💡 Décision de recrutement commercial"
        )

    # ════════════════════════════════════════════════════════════
    # TAB 5 — COÛT RÉEL D'UN RECRUTEMENT
    # ════════════════════════════════════════════════════════════
    with t5:
        st.markdown("<br>", unsafe_allow_html=True)
        comment(
            "Le vrai coût d'un recrutement n'est pas que le salaire. "
            "Il inclut le recrutement, la formation, la perte de productivité pendant la montée "
            "en compétence et le risque de départ. Ce simulateur calcule le coût complet.",
            "🧮 Coût réel d'un recrutement"
        )

        # Simulateur
        st.markdown(f'<div class="sec">⚙️  SIMULATEUR — Coût complet d\'un recrutement</div>',
                    unsafe_allow_html=True)
        col_s1, col_s2 = st.columns(2)
        with col_s1:
            poste_sim = st.selectbox("Poste recruté", [
                "Opérateur Machine", "Chef d'Équipe", "Technicien Maintenance",
                "Agent Qualité", "Commercial Terrain", "Magasinier", "Comptable"])
            salaires = {
                "Opérateur Machine": 185_000, "Chef d'Équipe": 290_000,
                "Technicien Maintenance": 330_000, "Agent Qualité": 250_000,
                "Commercial Terrain": 320_000, "Magasinier": 195_000, "Comptable": 280_000
            }
            sal = salaires[poste_sim]
            sal_total = sal * 1.35  # avec charges
            contrat = st.radio("Type de contrat", ["CDI", "Journalier", "Quinzainiers"], horizontal=True)
        with col_s2:
            mois_montee = st.slider("Mois de montée en compétence", 1, 6, 2)
            pct_perte   = st.slider("% de perte productivité pendant montée", 10, 60, 35)
            proba_depart= st.slider("Probabilité de départ dans les 12 mois (%)", 5, 50, 20)

        # Calcul
        cout_recrutement = sal * 0.5  # 50% du salaire mensuel (annonce, entretiens, temps RH)
        if contrat == "Journalier":
            sal_total = sal / 22 * 26 * 1.15  # journalier coûte + par mois travaillé
            cout_recrutement = sal * 0.15       # moins cher à recruter
        elif contrat == "Quinzainiers":
            sal_total = sal * 0.5 * 1.20
            cout_recrutement = sal * 0.10

        cout_formation       = mois_montee * 30_000  # formation + accompagnement
        cout_perte_prod      = sal_total * mois_montee * (pct_perte / 100)
        cout_risque_depart   = (cout_recrutement + cout_formation + cout_perte_prod) * (proba_depart / 100)
        cout_total_12_mois   = (sal_total * 12 + cout_recrutement + cout_formation +
                                cout_perte_prod + cout_risque_depart)
        cout_reel_mensuel    = cout_total_12_mois / 12

        st.markdown("<br>", unsafe_allow_html=True)
        section("📊  DÉCOMPOSITION DU COÛT RÉEL")

        postes_cout = [
            ("Salaires + Charges (12 mois)",      sal_total * 12,         C["bleu"]),
            ("Recrutement (annonce, entretiens)",  cout_recrutement,       C["violet"]),
            ("Formation & accompagnement",         cout_formation,         C["orange"]),
            ("Perte de productivité",              cout_perte_prod,        C["rose"]),
            ("Risque de départ (provisions)",      cout_risque_depart,     C["rouge"]),
        ]

        fig_cout = go.Figure()
        fig_cout.add_trace(go.Bar(
            x=[p[0] for p in postes_cout],
            y=[p[1] for p in postes_cout],
            marker_color=[p[2] for p in postes_cout],
            text=[fmt(p[1],"").strip() for p in postes_cout],
            textposition="outside", textfont=dict(size=10),
            hovertemplate="<b>%{x}</b><br>%{y:,.0f} FCFA<extra></extra>"))
        fig_cout = excel_style(fig_cout, 300, False)
        fig_cout.update_xaxes(tickangle=-15, tickfont=dict(size=9))
        fig_cout.update_yaxes(tickformat=",.0f", ticksuffix=" F")
        st.plotly_chart(fig_cout, use_container_width=True)

        # Résumé
        cr1,cr2,cr3,cr4 = st.columns(4)
        with cr1: kpi("💸","Coût Réel Total (12M)", fmt(cout_total_12_mois), "Au-delà du salaire seul","rouge")
        with cr2: kpi("📅","Coût Réel Mensuel",     fmt(cout_reel_mensuel),  "Inclut tout les coûts","orange")
        with cr3: kpi("💰","Salaire Seul (12M)",    fmt(sal_total*12),       "Ce que voit la compta","bleu")
        with cr4:
            surcharge = (cout_total_12_mois - sal_total*12) / (sal_total*12) * 100
            kpi("📊","Surcharge réelle",             f"+{surcharge:.0f}%",
                "Au-dessus du salaire brut","orange")

        prevision_comment(
            f"Pour un {poste_sim} en {contrat}, le coût réel sur 12 mois est "
            f"<b>{fmt(cout_total_12_mois)}</b> soit <b>{fmt(cout_reel_mensuel)}/mois</b>. "
            f"C'est <b>+{surcharge:.0f}%</b> de plus que le salaire seul. "
            f"Avant de recruter, vérifiez que ce poste génère au moins "
            f"{fmt(cout_reel_mensuel/0.15)} de CA mensuel (à 15% de marge brute).",
            "💡 Décision de recrutement — seuil de rentabilité"
        )

        # Comparatif CDI vs Journalier
        st.markdown("<br>", unsafe_allow_html=True)
        section("⚖️  COMPARATIF CDI vs JOURNALIER — Sur 12 mois pour ce poste")

        cdi_12 = sal * 1.35 * 12 + sal * 0.5 + mois_montee * 30_000 + sal*1.35*mois_montee*(pct_perte/100)
        jour_12= sal/22*26*1.15 * 12*0.7 + sal*0.15 + mois_montee*20_000 + sal/22*26*1.15*mois_montee*(pct_perte/100)*1.4
        # Journalier utilisé 70% du temps sur 12 mois + rebuts 40% plus élevés

        cout_rebut_extra = jour_12 * 0.008 * 45  # 0.8% rebut supplémentaire × coût matière estimé
        jour_12_total = jour_12 + cout_rebut_extra

        comp_data = pd.DataFrame({
            "Type": ["CDI", "Journalier (avec surcoût rebut)"],
            "Coût 12 mois": [cdi_12, jour_12_total],
            "Dont salaires": [sal*1.35*12, sal/22*26*1.15*12*0.7],
            "Dont rebuts extra": [0, cout_rebut_extra],
        })

        fig_comp = go.Figure()
        fig_comp.add_trace(go.Bar(x=comp_data["Type"], y=comp_data["Dont salaires"],
            name="Salaires + charges", marker_color=C["bleu"]))
        fig_comp.add_trace(go.Bar(x=comp_data["Type"],
            y=comp_data["Coût 12 mois"]-comp_data["Dont salaires"]-comp_data["Dont rebuts extra"],
            name="Recrutement + formation + perte prod.", marker_color=C["violet"]))
        fig_comp.add_trace(go.Bar(x=comp_data["Type"], y=comp_data["Dont rebuts extra"],
            name="Surcoût rebuts (qualité)", marker_color=C["rouge"]))
        fig_comp.update_layout(barmode="stack")
        fig_comp = excel_style(fig_comp, 300)
        fig_comp.update_yaxes(tickformat=",.0f", ticksuffix=" F")
        st.plotly_chart(fig_comp, use_container_width=True)

        diff = jour_12_total - cdi_12
        st.markdown(f"""
        <div class="{'alert-r' if diff > 0 else 'alert-g'}">
          {'⚖️' if diff > 0 else '✅'}
          <div>
            <b>{'Le journalier coûte finalement ' + fmt(abs(diff)) + ' de PLUS sur 12 mois' if diff > 0
               else "Le CDI est plus économique : " + fmt(abs(diff)) + " d'économie sur 12 mois"}</b>
            <small>Le journalier semble moins cher au mois, mais les rebuts supplémentaires,
            la moindre productivité et les coûts de renouvellement fréquent effacent l'avantage.
            À réserver aux pics de production courts (moins de 2 mois).</small>
          </div>
        </div>""", unsafe_allow_html=True)


# ── PAGE RENTABILISATION ──────────────────────────────────────
elif page == "💡  Rentabilisation":

    st.markdown(f"""
    <div style="background:linear-gradient(135deg,#0F172A,{C["or"]}44,{C["orange"]}88);
                border-radius:14px;padding:22px 28px;margin-bottom:18px;
                border:1px solid rgba(217,119,6,0.4);">
      <div style="font-size:1.15rem;font-weight:800;color:white;margin-bottom:4px;">
        💡 Comment MULTIPACK SA peut gagner plus — Guide Complet de Rentabilisation</div>
      <div style="font-size:0.8rem;color:rgba(255,255,255,0.65);">
        Chaque levier expliqué · Gain chiffré · Priorité · Étapes concrètes</div>
    </div>""", unsafe_allow_html=True)
    ...
    val_stock = df_inv["Valeur"].sum()
    rend_moy  = df_prod["Taux rendement"].mean()
    rfm_data  = calculer_rfm(df_f)
    ca_dorm   = rfm_data[rfm_data["Catégorie"]=="Dormants"]["Montant"].sum()
    ca_risk   = rfm_data[rfm_data["Catégorie"]=="A Risque"]["Montant"].sum()
    ca_champ  = rfm_data[rfm_data["Catégorie"].isin(["Champions","Fidèles"])]["Montant"].sum()

    comment("Cette page répond à la question de la direction : "
            "<b>Comment gagner plus d'argent avec ce qu'on a déjà ?</b> "
            "Chaque recommandation est chiffrée sur vos données réelles, expliquée et priorisée. "
            "Aucun investissement majeur requis pour les leviers urgents.","💡 Mode d'emploi")

    ACTIONS = [
        {"ico":"💸","titre":"Recouvrer les impayés","cls":"rc-orange","prio":"🔴 URGENT — Cette semaine",
         "gain":ca_impaye*0.40,
         "desc":f"MULTIPACK a <b>{fmt(ca_impaye)}</b> de factures non payées. C'est de l'argent déjà gagné, pas encore encaissé. Récupérer 40% = <b>{fmt(ca_impaye*0.40)}</b> de trésorerie immédiate.",
         "pourquoi":"C'est le levier le plus rapide. L'argent est acquis comptablement. Chaque jour de retard augmente le risque de perte définitive.",
         "comment":"Relances à J+30, J+45, J+60. Au-delà : lettre recommandée. Proposer un échéancier aux clients en difficulté.",
         "etapes":["Appeler cette semaine les 5 plus gros impayés (voir page 💳 Paiements)",
                   "Envoyer lettre de relance formelle aux impayés > 60 jours",
                   "Proposer un échéancier aux clients en difficulté",
                   "Exiger 30% d'acompte sur nouvelles commandes > 500K FCFA"]},
        {"ico":"🎯","titre":"Réactiver les clients dormants","cls":"rc-vert","prio":"🟡 PRIORITÉ — Ce mois",
         "gain":ca_dorm*0.35,
         "desc":f"Des clients avec un bon historique n'ont pas commandé récemment. Réactiver 35% d'entre eux : <b>{fmt(ca_dorm*0.35)}</b> de CA récupéré.",
         "pourquoi":"Un client réactivé coûte 5x moins cher qu'un nouveau client. Ces clients connaissent déjà MULTIPACK.",
         "comment":"Email personnalisé + appel + offre de retour (remise 10-15% sur la première commande).",
         "etapes":["Identifier les clients dormants (page 🎯 RFM)",
                   "Préparer une offre de retour exclusive",
                   "Appeler personnellement le top 10 des dormants par CA historique",
                   "Mettre en place un email automatique à 90j sans commande"]},
        {"ico":"📈","titre":"Upsell Champions & Fidèles","cls":"rc-bleu","prio":"🟡 PRIORITÉ — Ce mois",
         "gain":ca_champ*0.12,
         "desc":f"Les meilleurs clients achètent souvent sur une seule gamme. Upsell 12% = <b>{fmt(ca_champ*0.12)}</b> additionnel.",
         "pourquoi":"Il est 6x plus facile de vendre plus à un client existant qu'à un nouveau.",
         "comment":"Proposer systématiquement un produit complémentaire. Offres volume avec paliers.",
         "etapes":["Identifier Champions n'achetant qu'une gamme (page 🎯 RFM)",
                   "Préparer des offres cross-sell (ex: client Sacs → proposer Gobelets)",
                   "Mettre en place des remises volume par palier (10/20/30%)",
                   "Former l'équipe commerciale à la vente additionnelle"]},
        {"ico":"⚙️","titre":"Améliorer le rendement machines","cls":"rc-violet","prio":"🟢 MOYEN TERME — Ce trimestre",
         "gain":max(0, ca_total*(0.90-rend_moy/100)*0.08),
         "desc":f"Rendement moyen : <b>{rend_moy:.1f}%</b> (objectif 90%). Chaque rebut = matière + temps machine perdus. Passer à 90% = produire plus sans nouvel équipement.",
         "pourquoi":"Les rebuts coûtent deux fois : achat matières + temps machine perdu.",
         "comment":"Audit technique, maintenance préventive planifiée, formation des opérateurs.",
         "etapes":["Identifier les 2 machines avec taux rebut le plus élevé (page 🏭 Production)",
                   "Planifier révision technique dans les 30 prochains jours",
                   "Mettre en place contrôles qualité en cours de production",
                   "Nommer un opérateur référent qualité par machine"]},
        {"ico":"📦","titre":"Optimiser les stocks","cls":"rc-or","prio":"🟢 MOYEN TERME — Ce trimestre",
         "gain":val_stock*0.08,
         "desc":f"Stock actuel : <b>{fmt(val_stock)}</b>. Réduire les sur-stocks de 8% libère <b>{fmt(val_stock*0.08)}</b> de trésorerie sans impacter le service client.",
         "pourquoi":"Stock excessif = entrepôt, risque obsolescence, trésorerie bloquée. L'argent libéré peut financer autre chose.",
         "comment":"Analyser références avec stock > 6 mois de ventes. Commander plus souvent en plus petites quantités.",
         "etapes":["Identifier les sur-stocks (page 📦 Stocks & Inventaire)",
                   "Définir stock cible par référence : 1.5 mois de ventes",
                   "Commander plus fréquemment en quantités plus petites",
                   "Utiliser les prévisions (page 🔮) pour anticiper les besoins"]},
        {"ico":"🗺️","titre":"Ouvrir de nouvelles zones géographiques","cls":"rc-vert","prio":"🔵 STRATÉGIQUE — Semestre prochain",
         "gain":ca_total*0.18,
         "desc":f"Nord, Est et Ouest sont sous-représentés. 2-3 nouveaux distributeurs locaux peuvent générer <b>{fmt(ca_total*0.18)}</b> additionnel.",
         "pourquoi":"MULTIPACK est concentré sur Abidjan. Les villes secondaires (Bouaké, Korhogo, Daloa) sont des marchés non exploités.",
         "comment":"Recruter agents commerciaux locaux à la commission. Adapter les délais de livraison.",
         "etapes":["Étudier la demande dans Bouaké, Korhogo, Daloa",
                   "Identifier des grossistes locaux comme distributeurs partenaires",
                   "Proposer des conditions attractives pour les premiers partenaires",
                   "Démarrer avec Sacs Poubelles + Gobelets (produits à forte demande universelle)"]},
        {"ico":"🌿","titre":"Gamme Eco-Responsable","cls":"rc-bleu","prio":"🔵 STRATÉGIQUE — Semestre prochain",
         "gain":ca_total*0.10,
         "desc":f"Les grandes surfaces veulent réduire le plastique non recyclable. Gamme biosourcée facturée 20-30% plus cher. Potentiel : <b>{fmt(ca_total*0.10)}</b>.",
         "pourquoi":"Tendance de fond. Les premiers positionnés capturent la prime de prix et fidélisent les clients sensibles à l'environnement.",
         "comment":"Ligne MULTIPACK ECO : sacs biosourcés, gobelets recyclés. Communiquer sur la traçabilité.",
         "etapes":["Tester 2 références eco auprès de la Grande Distribution",
                   "Calculer coût de revient et fixer prix avec marge cible",
                   "Obtenir une certification environnementale",
                   "Lancer campagne commerciale dédiée"]},
    ]

    gain_total = sum(a["gain"] for a in ACTIONS)
    st.markdown(f"""
    <div style="background:linear-gradient(135deg,{C["sidebar"]},{C["or"]}22);
                border:2px solid {C["or"]}55;border-radius:16px;
                padding:24px 28px;margin:0 0 24px;text-align:center;">
      <div style="font-size:0.72rem;font-weight:800;color:{C["or"]};text-transform:uppercase;letter-spacing:0.1em;">
        💰 POTENTIEL DE GAIN TOTAL IDENTIFIÉ</div>
      <div style="font-size:3rem;font-weight:900;color:white;letter-spacing:-0.04em;margin:10px 0 6px;">
        {fmt(gain_total)}</div>
      <div style="font-size:0.85rem;color:rgba(255,255,255,0.5);">
        Calculé sur vos données réelles · Sans investissement majeur</div>
    </div>""", unsafe_allow_html=True)

    section("🚀  LES 7 LEVIERS — Par ordre de priorité et d'impact")
    for i in range(0, len(ACTIONS), 2):
        c2 = st.columns(2)
        for j, act in enumerate(ACTIONS[i:i+2]):
            with c2[j]:
                etapes_html = "".join(
                    f'<div class="rc-step"><span class="rs-num">{k+1}</span><span>{e}</span></div>'
                    for k,e in enumerate(act["etapes"]))
                st.markdown(f"""
                <div class="renta-card {act["cls"]}">
                  <span style="font-size:2rem;margin-bottom:10px;display:block;">{act["ico"]}</span>
                  <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:4px;">
                    <div style="font-size:1rem;font-weight:800;color:{C["texte"]};">{act["titre"]}</div>
                    <span style="font-size:0.65rem;font-weight:700;background:{C["gris_bg"]};
                                 border:1px solid {C["bordure"]};border-radius:20px;padding:2px 8px;
                                 color:{C["muted"]};white-space:nowrap;margin-left:6px;">{act["prio"]}</span>
                  </div>
                  <div style="font-size:1.4rem;font-weight:900;color:{C["vert"]};margin:8px 0;">
                    + {fmt(act["gain"],"").strip()} FCFA</div>
                  <div style="font-size:0.78rem;color:{C["muted"]};line-height:1.55;margin-bottom:10px;">{act["desc"]}</div>
                  <div style="font-size:0.7rem;font-weight:800;color:{C["muted"]};text-transform:uppercase;letter-spacing:0.07em;margin-bottom:4px;">💡 Pourquoi ?</div>
                  <div style="font-size:0.78rem;color:{C["muted"]};font-style:italic;line-height:1.5;margin-bottom:10px;">{act["pourquoi"]}</div>
                  <div style="font-size:0.7rem;font-weight:800;color:{C["muted"]};text-transform:uppercase;letter-spacing:0.07em;margin-bottom:4px;">⚙️ Comment ?</div>
                  <div style="font-size:0.78rem;color:{C["muted"]};line-height:1.5;margin-bottom:10px;">{act["comment"]}</div>
                  <div style="font-size:0.7rem;font-weight:800;color:{C["muted"]};text-transform:uppercase;letter-spacing:0.07em;margin-bottom:6px;">✅ Étapes concrètes</div>
                  {etapes_html}
                </div>""", unsafe_allow_html=True)
        st.markdown("<div style='margin:10px 0;'></div>", unsafe_allow_html=True)

    st.markdown("<br>", unsafe_allow_html=True)
    section("📊  RÉCAPITULATIF FINANCIER — Gains par levier")
    rec_df = pd.DataFrame([(a["titre"],a["gain"]) for a in ACTIONS],columns=["Levier","Gain"])
    rec_df = rec_df.sort_values("Gain",ascending=True)
    fig_rec = go.Figure(go.Bar(
        x=rec_df["Gain"],y=rec_df["Levier"],orientation="h",
        marker_color=[C["orange"],C["vert"],C["bleu"],C["violet"],C["or"],C["vert"],C["bleu"]],
        text=rec_df["Gain"].apply(lambda v: fmt(v,"").strip()),
        textposition="outside",textfont=dict(size=11),
        hovertemplate="<b>%{y}</b><br>%{x:,.0f} FCFA<extra></extra>"))
    fig_rec = excel_style(fig_rec,380,False)
    fig_rec.update_xaxes(tickformat=",.0f")
    st.plotly_chart(fig_rec, use_container_width=True)

    gain_court = ACTIONS[0]["gain"]+ACTIONS[1]["gain"]+ACTIONS[2]["gain"]
    prevision_comment(
        f"Les 3 leviers urgents (impayés, réactivation, upsell) peuvent générer "
        f"<b>{fmt(gain_court)}</b> dans les 3 prochains mois sans investissement. "
        f"Le plan complet sur 12 mois représente <b>{fmt(gain_total)}</b> de potentiel identifié.",
